...
function(rm)
   ...
   local j = 0;
   local locone, loctwo, locthree;
   for i = 1, 10000 do
      locone = selection.room():rndcoord(0);
      loctwo = selection.room():rndcoord(0);
      locthree = selection.room():rndcoord(0);
      if locone.x + locthree.x == loctwo.x + loctwo.x then
         if locone.y + locthree.y == loctwo.y + loctwo.y then
             if math.abs(locone.x - loctwo.x) >= 2 or math.abs(locone.y - loctwo.y) >= 2 then
                 des.altar({ coord = locone, align = align[array[1]] });
                 des.altar({ coord = loctwo, align = align[array[2]] });
                 des.altar({ coord = locthree, align = align[array[3]] });
                 j = 1;
                 break;
             end                 
         end

...
deadbook(struct obj* book2)
{
 ...
 raise_dead:
        ...
        if (!rn2(3) && ((mtmp = makemon(&mons[PM_MASTER_LICH], u.ux, u.uy,
                                        NO_MINVENT)) != 0
                        || (mtmp = makemon(&mons[PM_MARID], u.ux, u.uy,
                                           NO_MINVENT)) != 0)) {
            mtmp->mpeaceful = 0;
            set_malign(mtmp);
        }
...
int
spell_hit_dice(int caster_level)
{
    int i = 0;
    for (int j = 1; j < 15; j++) {
        i += j;
    ...
    debugpline1("caster_level = %d", caster_level);
    return 15;
}
...
spelleffects(int spell_otyp, boolean atme, boolean force)
{
    ...
    case SPE_CONE_OF_COLD:
                ...
                while (n--) {
                    if (!u.dx && !u.dy && !u.dz) {
                        ...
                        explode(u.dx, u.dy,
                                ...
                                spell_damage_bonus(spell_hit_dice(u.ulevel)), 0,
        ...
        /* if (role_skill == P_BASIC || !u.uswallow) {
            if (throwspell()) {
                cc.x = u.dx;
                cc.y = u.dy;
                int angle;
                ...
                for (x = -rc; x <= rc; x++)
                    for (y = -rc; y <= rc; y++) {
                        angletwo = (double) 180 * atan2(y, x) / M_PI;
                        if ((900 + angle - angletwo)) % 360
                    }
                        
                while (n--) {
                    if (!isok(u.dx, u.dy) || IS_STWALL(levl[u.dx][u.dy].typ))
                        continue;
...
can_center_spell_location(coordxy x, coordxy y)
{
    ...
    if (u.uswallow)
        return FALSE;
    struct monst *mtmp;
    if (((x != u.ux || y != u.uy) && !cansee(x, y)
                && (!(mtmp = m_at(x, y)) || !canspotmon(mtmp)))
...
throwspell(int breath)
{
    ...
    if (breath)
        pline("Where do you want to aim your breath?");
    else
    ...
    if (distmin(u.ux, u.uy, cc.x, cc.y) > 10) {
        if (breath)
            Your("breath dissipates over the distance!");
        else

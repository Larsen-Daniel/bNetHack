...
find_pmmonst(int pm)
{
    ...
    if ((gm.mvitals[pm].mvflags & G_BANID) == 0)
...
bee_eat_jelly(struct monst* mon, struct obj* obj)
{
    ...
    if (!mtmp) {
        ...
        if (DEADMONSTER(mon))
            return 1; /* dead; apparently queen bees have been banished */
        mon->mfrozen = m_delay, mon->mcanmove = 0;
        mon->mfrozenr = REASON_EMPLOYED;
    ...
    boolean fleemsg)
{
    ...
    if (!first || !mtmp->mflee) {
        ...
        if (mtmp->data == &mons[PM_UTUKKU] && !mtmp->mspec_used) {
...
dochug(register struct monst* mtmp)
{
    ...
    if (is_covetous(mdat)) {
        if (In_hell(&u.uz) && !Is_sanctum(&u.uz) && noteleport_level(mtmp))
            ;
        else {
            (void) tactics(mtmp);
    ...
    if ((!mtmp->mpeaceful || Conflict) && inrange
        ...
        if (!(scared && mw_tmp && is_pick(mw_tmp))
            && (mtmp->weapon_check == NEED_WEAPON || mtmp->weapon_check == NEED_HTH_WEAPON)
    ...
    /* Tarasques create clouds of stench. This does not cost a move. */
    if (mtmp->data == &mons[PM_TARASQUE]) /* stench */
            ...
            for (a = &mdat->mattk[0]; a < &mdat->mattk[NATTK]; a++) {
                if (a->aatyp == AT_MAGC
                    && (a->adtyp == AD_SPEL || a->adtyp == AD_CLRC
                        || a->adtyp == AD_ELVN || a->adtyp == AD_DJIN)) {
    ...
    if (inrange && mtmp->data->msound == MS_CUSS && !mtmp->mpeaceful
        ...
        cuss(mtmp);
    if (inrange && mtmp->data->msound == MS_BOOM && !mtmp->mpeaceful
        && couldsee(mtmp->mx, mtmp->my) && !mtmp->minvis && !rn2(5))
        boom(mtmp);
...
m_move(register struct monst* mtmp, register int after)
{
    ...
    if (mtmp->guardingsquare && mtmp->mx == mtmp->mfixedx && mtmp->my == mtmp->mfixedy)
        return MMOVE_NOTHING; /* got to continue guarding the square */
...
set_apparxy(register struct monst* mtmp)
{
    ...
    notthere = (Displaced && mtmp->data != &mons[PM_DISPLACER_BEAST] && mtmp->data != &mons[PM_BABY_SHIMMERING_DRAGON] && mtmp->data != &mons[PM_SHIMMERING_DRAGON]);
...
{
    if (!(gm.mvitals[PM_FOG_CLOUD].mvflags & G_BANID) && is_vampshifter(mtmp)

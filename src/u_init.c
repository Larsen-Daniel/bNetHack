...
static void makewizardspells(void);
...
static struct trobj Wizspells[] = {
    { SPE_FORCE_BOLT, 0, SPBOOK_CLASS, 1, UNDEF_BLESS },
    { SPE_FORCE_BOLT, 0, SPBOOK_CLASS, 1, UNDEF_BLESS },
    { SPE_FORCE_BOLT, 0, SPBOOK_CLASS, 1, UNDEF_BLESS },
    { 0, 0, 0, 0, 0 }
};
...
u_init(void)
{
    ...
    for (i = 0; i < A_MAX; i++)
    {
        if (AMAX(i) >= 16) {
            ABASE(i) = AMAX(i) = 16;
            (void) adjattrib(i, rn2(3) - 2, TRUE);
            (void) adjattrib(i, rn2(3) - 1, TRUE);
            if (ABASE(i) < AMAX(i))
                AMAX(i) = ABASE(i);
        }
        /* {
            (void) adjattrib(i, rn2(3) - 1, TRUE);
            (void) adjattrib(i, rn2(4) - 2, TRUE);
            if (ABASE(i) < AMAX(i))
                AMAX(i) = ABASE(i);
        }
        if (AMAX(i) >= 16)
            if (i == 0)
                (void) adjattrib(i, rn2(2) - 1, TRUE); */
        ...
        if (ABASE(i) > 18) {
            (void) adjattrib(i, 18 - ABASE(i), TRUE);
            if (ABASE(i) < AMAX(i))
                AMAX(i) = ABASE(i);
        }
    }
...
{
    struct obj *obj[3];
    int otyp, i, j, lowestskill, lowestskillnum;
    int otyps[3];
    int spellcount;
    for(;;) {
        for (i = 0; i < 3; i++) {
            obj[i] = mkobj(SPBOOK_CLASS, FALSE);
            otyp = obj[i]->otyp;
            while ((otyp == SPE_FORCE_BOLT && Role_if(PM_WIZARD))
                   /* powerful spells are either useless to
                      low level players or unbalancing; also
                      spells in restricted skill categories */
                   || (obj[i]->oclass == SPBOOK_CLASS
        ...
        for (i = 0; i < 3; i++)
            if (objects[obj[i]->otyp].oc_level == 1)
                break;
        if (i == 3)
            continue;
        spellcount = 0;
        ...
        if (spellcount > 1)
            continue;
        if (objects[obj[0]->otyp].oc_skill == objects[obj[1]->otyp].oc_skill)
            continue;
        if (objects[obj[0]->otyp].oc_skill == objects[obj[2]->otyp].oc_skill)
            continue;
        if (objects[obj[1]->otyp].oc_skill == objects[obj[2]->otyp].oc_skill)
            continue;
        otyps[0] = obj[0]->otyp;
        otyps[1] = obj[1]->otyp;
        otyps[2] = obj[2]->otyp;
        for (j = 0; j < 3; j++) {
            lowestskill = -1;
            for (i = 0; i < 3; i++)
                if (otyps[i] != -1 && otyps[i] > lowestskill) {
                    lowestskill = otyps[i];
                    lowestskillnum = i;
                }
            Wizspells[2 - j].trotyp = lowestskill;
            otyps[lowestskillnum] = -1;
        }
        ini_inv(Wizspells);
        break;
    }
...
ini_inv(struct trobj *trop)
{
    ...
    while (trop->trclass) {
        ...
        if (otyp != UNDEF_TYP) {
            ...
            if (trop->trclass == SPBOOK_CLASS && Role_if(PM_WIZARD)) {
                makewizardspells();
                return;
            }
            int trycnt = 0;

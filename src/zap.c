...
bhitm(struct monst *mtmp, struct obj *otmp)
{
    ...
    case SPE_TELEPORT_AWAY:
        if (disguised_mimic)
            seemimic(mtmp);
        reveal_invis = !u_teleport_mon(mtmp, TRUE, FALSE);
        break;
    ...
    case SPE_EXTRA_HEALING:
        ...
        if (mtmp->data != &mons[PM_PESTILENCE]) {
            ...
            mtmp->mhp += d(6 * (1 + (Role_if(PM_HEALER) && u.uhave.questart)), otyp == SPE_EXTRA_HEALING ? 8 : 4);
            ...
            (void) resist(mtmp, otmp->oclass,
                          d(3 * (1 + (Role_if(PM_HEALER) && u.uhave.questart)), otyp == SPE_EXTRA_HEALING ? 8 : 4), TELL);
        }
        break;
...
cancel_item(struct obj *obj)
{
    ...
    if (is_art(obj, ART_SCEPTRE_OF_MIGHT) && u_wield_art(ART_SCEPTRE_OF_MIGHT) && strength_type && sceptre_strength && Role_if(PM_CAVE_DWELLER)) {
        if (obj->spe>0) {
            ABON(A_STR) -= obj->spe;
            gc.context.botl = TRUE;
        }
...
create_polymon(struct obj *obj, int okind)
{
    ...
    case METAL:
    case MITHRIL:
            pm_index = rn2(10) ? PM_IRON_GOLEM : PM_STEEL_GOLEM;
        material = "metal ";
        break;
    case COPPER:
    case SILVER:
    ...
    if (!(gm.mvitals[pm_index].mvflags & G_BANID))
        mdat = &mons[pm_index];
...
stone_to_flesh_obj(struct obj *obj)
{
    ...
    case TOOL_CLASS: /* figurines */
        if (obj->otyp == BOULDER) {
            ...
            } else if ((ptr->bani & (G_NOCORPSE | G_UNIQ)) != 0) {
                /* didn't revive but can't leave corpse either */
                res = 0;
            } else {
                /* unlikely to get here since banishing monsters also
...
dozap(void)
{
    ...
    if (!canusehands) {
        if (nohands(gy.youmonst.data))
            ...
            You("aren't able to zap anything in your current shape.");
        return ECMD_OK;
    }
...
zapyourself(struct obj *obj, boolean ordinary)
{
    ...
    case WAN_LIGHTNING:
        ...
        if (!Shock_resistance) {
            ...
            damage = resist_damage(AD_ELEC, damage);
        }
    ...
    case FIRE_HORN:
        ...
        if (Fire_resistance) {
            ...
            damage = resist_damage(AD_FIRE, damage);
        }
        burn_away_slime();
    ...
    case FROST_HORN:
        ...
        if (Cold_resistance) {
            shieldeff(u.ux, u.uy);
            You_feel("rather cold.");
            monstseesu(M_SEEN_COLD);
            ugolemeffects(AD_COLD, damage);
            damage = resist_damage(AD_COLD, damage);
        } else {
    ...
    case WAN_MAKE_INVISIBLE: {
        ...
        incr_itimeout(&HInvis, rn1(25, 50));
        if (msg) {
            learn_it = TRUE;
            newsym(u.ux, u.uy);
            self_invis_message();
    ...
    case SPE_EXTRA_HEALING:
        learn_it = TRUE; /* (no effect for spells...) */
        healup(d(6 * (1 + (Role_if(PM_HEALER) && u.uhave.questart)), obj->otyp == SPE_EXTRA_HEALING ? 8 : 4), 0, FALSE,
...
weffects(struct obj *obj)
{
    ...
    if (u.usteed && (objects[otyp].oc_dir != NODIR) && !u.dx && !u.dy
        ...
        if (otyp == WAN_DIGGING || otyp == SPE_DIG)
            ...
            ubuzz(BZ_U_SPELL(BZ_OFS_SPE(otyp)), spell_hit_dice(u.ulevel));
...
zhitu(int type, int nd, const char *fltxt, coordxy sx, coordxy sy)
{
    ...
    case ZT_LIGHTNING:
        ...
        if (Shock_resistance) {
            shieldeff(sx, sy);
            You("aren't too badly shaken up.");
            monstseesu(M_SEEN_ELEC);
            ugolemeffects(AD_ELEC, dam);
            dam = resist_damage(AD_ELEC, dam);
        } else {
            exercise(A_CON, FALSE);
        }
        if (!rn2(3))
    ...
    case ZT_ACID:
        dam = d(nd, 6);
        if (Acid_resistance) {
            pline_The("%s burns somewhat.", hliquid("acid"));
            monstseesu(M_SEEN_ACID);
            dam = resist_damage(AD_ACID, dam);
        } else {
    ...
    boolean say) /* announce out of sight hit/miss events if true */
{
 ...
 buzzmonst:
            ...
            nomul(0);
            if (u.usteed && !rn2(3)) {
                if (!mon_reflects(u.usteed, strcat(The(flash_str(fltyp, FALSE)), " reflects from %s %s!")))
                {
                    mon = u.usteed;
                    goto buzzmonst;
                }
...
u_adtyp_resistance_obj(int dmgtyp)
{
    ...
    if ((u.uprops[prop].extrinsic & (W_ARMOR | W_ACCESSORY | W_WEP | W_SWAPWEP)) != 0)
        return TRUE;
...
static int
base_resist(struct monst *mtmp, char oclass, int damage, int tell, int mr)
{
    int resisted;
    int alev, dlev;
...
{
    return base_resist(mtmp, oclass, damage, tell, mtmp->data->mr);
}

int
smartresist(struct monst *mtmp, char oclass, int damage, int tell)
{
    if (mtmp->data == &mons[PM_MARID])
        return base_resist(mtmp, oclass, damage, tell, 10);
    if (mtmp->data == &mons[PM_DJINNI])
        return base_resist(mtmp, oclass, damage, tell, 100);
    return base_resist(mtmp, oclass, damage, tell, mtmp->data->mr);
}

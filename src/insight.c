...
/*
 * Enlightenment and Conduct+Achievements and Vanquished+Extinct+Bani'd
...
one_characteristic(int mode, int final, int attrindx)
{
    ...
    if (!hide_innate_value) {
        ...
        if (!strength_type)
            ...
            interesting_alimit = final ? TRUE : (alimit != 18);
...
status_enlightenment(int mode, int final)
{
    ...
    if (Pinned && !nohands(gy.youmonst.data)) {
        const char *bp = makeplural(body_part(ARM));
        char yourarms[BUFSZ];
        sprintf(yourarms,"Your %s", bp);
        enl_msg(yourarms, " are ", " were ", "pinned", "");
    }
    if (Wounded_legs) {
...
attributes_enlightenment(int unused_mode UNUSED, int final)
{
    ...
    if (Serendipitous)
        you_have("the world on your side", from_what(SERENDIPITOUS));
    if (Unserendipitous)
        enl_msg("The world ", "is", "was", " against you", "");
    if (Ferocious)
        you_are("almost unstoppable", from_what(FEROCIOUS));
...
show_conduct(int final)
{
    ...
    nbanished = num_banishments();
    if (nbanished == 0) {
        you_have_never("banished any monsters");
    } else {
        Sprintf(buf, "banished %d type%s of monster%s", nbanished,
                plur(nbanished), plur(nbanished));
        you_have_X(buf);
    }
    ...
    const genericptr vptr2)
{
    ...
    case VANQ_MSTR_MNDX:
        ...
        break;
    case VANQ_ALPHA_SEP:
        uniq1 = ((mons[indx1].bani & G_UNIQ) && indx1 != PM_HIGH_CLERIC);
        uniq2 = ((mons[indx2].bani & G_UNIQ) && indx2 != PM_HIGH_CLERIC);
    ...
    for (i = LOW_PM; i < NUMMONS; i++)
        if (gm.mvitals[i].born || gm.mvitals[i].died
            ...
            Sprintf(buf, fmt,
                    ...
                    : ((gm.mvitals[i].mvflags & G_GONE) == G_BANID) ? 'G'
...
#define UniqCritterIndx(mndx) ((mons[mndx].bani & G_UNIQ) \
                               && mndx != PM_HIGH_CLERIC)
...
/* number of monster species which have been banished */
int
num_banishments(void)
{
    int i, n = 0;

    for (i = LOW_PM; i < NUMMONS; ++i) {
        if (gm.mvitals[i].mvflags & G_BANID) {
            ++n;
            if (UniqCritterIndx(i))
                impossible("unique creature '%d: %s' banished?",
...
void
list_banished(char defquery, boolean ask)
{
    register int i;
    int nbanished, nextinct;
    char c;
    winid klwin;
    char buf[BUFSZ];
    boolean dumping; /* for DUMPLOG; doesn't need to be conditional */
    ...
    /* banished or extinct species list */
    if (nbanished != 0 || nextinct != 0) {
        Sprintf(buf, "Do you want a list of %sspecies%s%s?",
                (nextinct && !nbanished) ? "extinct " : "",
                (nbanished) ? " banished" : "",
                (nextinct && nbanished) ? " and extinct" : "");
        ...
        if (c == 'y') {
            ...
            Sprintf(buf, "%s%s species:",
                    (nbanished) ? "Banished" : "Extinct",
                    (nextinct && nbanished) ? " or extinct" : "");
            putstr(klwin, 0, buf);
            if (!dumping)
                putstr(klwin, 0, "");

            for (i = LOW_PM; i < NUMMONS; i++) {
                /* uniques can't be banished but can become extinct;
            ...
            if (!dumping)
                putstr(klwin, 0, "");
            if (nbanished > 0) {
                Sprintf(buf, "%d species banished.", nbanished);
                putstr(klwin, 0, buf);
            }
            if (nextinct > 0) {
        ...
        putstr(0, 0, "No species were banished or became extinct.");
#endif
    }
}
...
/* probing applied to monster -- one-line feedback */
void
...
/* stethoscope applied to monster -- one-line feedback */
void
msteth(struct monst *mtmp)
{
    char info[BUFSZ], monnambuf[BUFSZ];
    ...
    if (mtmp->mblinded)
        Strcat(info, ", blinded");
    if (mtmp->msleeping || (mtmp->mfrozen && mtmp->mfrozenr == REASON_SLEEPING))
        Strcat(info, ", asleep");
    if (mtmp->mtame) {
        if (gm.moves > EDOG(mtmp)->hungrytime + DOG_STARVE)
            Strcat(info, ", starving");
        else if (gm.moves > EDOG(mtmp)->hungrytime + DOG_WEAK)
            Strcat(info, ", weak from hunger");
        else if (gm.moves > EDOG(mtmp)->hungrytime + DOG_HUNGRY)
            Strcat(info, ", hungry");
    }


    /* unlike a normal mon_nam, use "saddled" even if it has a name */
    Strcpy(monnambuf, x_monnam(mtmp, ARTICLE_THE, (char *) 0,
                               SUPPRESS_IT, FALSE));
    if (Role_if(PM_HEALER))
        pline("Status of %s: HP %d(%d)%s.", monnambuf, mtmp->mhp, mtmp->mhpmax, info);
    else {
        int msgquality = 0;
        if (mtmp->mhp * 6 > mtmp->mhpmax * 5) {
            if (mtmp->mtame && gm.moves > EDOG(mtmp)->hungrytime + DOG_HUNGRY)
                pline("%c%s seems to be malnourished.", highc(monnambuf[0]), monnambuf + 1);
            else {
                pline("%c%s seems to be in perfect health.", highc(monnambuf[0]), monnambuf + 1);
                msgquality = 1;
            }
        }
        else if (mtmp->mhp * 6 > mtmp->mhpmax * 4) {
            if (mtmp->mtame && gm.moves > EDOG(mtmp)->hungrytime + DOG_HUNGRY)
                pline("%c%s seems to be malnourished.", highc(monnambuf[0]), monnambuf + 1);
            else {
                pline("%c%s seems to be in excellent health.", highc(monnambuf[0]), monnambuf + 1);
                msgquality = 1;
            }
        }
        else if (mtmp->mhp * 6 > mtmp->mhpmax * 3) {
            if (mtmp->mtame && gm.moves > EDOG(mtmp)->hungrytime + DOG_HUNGRY)
                pline("%c%s seems to be malnourished.", highc(monnambuf[0]), monnambuf + 1);
            else {
                pline("%c%s seems to be in fine health.", highc(monnambuf[0]), monnambuf + 1);
                msgquality = 1;
            }
        }
        else if (mtmp->mhp * 6 > mtmp->mhpmax * 2)
            pline("%c%s seems to be in poor health.", highc(monnambuf[0]), monnambuf + 1);
        else if (mtmp->mhp * 6 > mtmp->mhpmax)
            pline("%c%s seems to be in very poor health.", highc(monnambuf[0]), monnambuf + 1);
        else
            pline("%c%s seems to be at death's door.", highc(monnambuf[0]), monnambuf + 1);
        if (msgquality == 1) {
            if (mtmp->mblinded) {
                if (mtmp->msleeping || (mtmp->mfrozen && mtmp->mfrozenr == REASON_SLEEPING))
                    pline("However, %s has been blinded and is currently sleeping.",monnambuf);
                else
                    pline("However, %s has been blinded.",monnambuf);
            }
            else
                if (mtmp->msleeping || (mtmp->mfrozen && mtmp->mfrozenr == REASON_SLEEPING))
                    pline("However, %s is asleep.",monnambuf);
        }
        else {
            if (mtmp->mblinded) {
                if (mtmp->msleeping || (mtmp->mfrozen && mtmp->mfrozenr == REASON_SLEEPING))
                    pline("Moreover, %s has been blinded and is currently sleeping.",monnambuf);
                else
                    pline("Moreover, %s has been blinded.",monnambuf);
            }
            else
                if (mtmp->msleeping || (mtmp->mfrozen && mtmp->mfrozenr == REASON_SLEEPING))
                    pline("Moreover, %s is asleep.",monnambuf);
        }
    }
}

/* probing applied to hero -- one-line feedback */
void
ustatusline(void)
{
    char info[BUFSZ];
...
/* stethoscope applied to hero -- one-line feedback */
void
usteth(void)
{
    char info[BUFSZ];
    ...
    pline("Status of %s:  HP %d(%d)%s.", gp.plname, Upolyd ? u.mh : u.uhp,
          Upolyd ? u.mhmax : u.uhpmax, info);
}

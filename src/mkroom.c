...
morguemon(void)
{
    ...
    if (hd > 10 && (i < 10 || (i >= 25 && i < 50))) {
        if (Inhell || In_endgame(&u.uz)) {
            return mkclass(S_DEMON, 0);
        } else {
            int ndemon_res = ndemon(A_NONE, (struct permonst *) 0);
...
antholemon(void)
{
    ...
    do {
        ...
        case 0:
            if (rn2(level_difficulty()) > 8 && rn2(100) >= 40) {
                mtyp = PM_BULLET_ANT;
            }
            else {
                mtyp = PM_SOLDIER_ANT;
        ...
        /* try again if chosen type has been banished or used up */
                ...
                if (!OBJ_AT(sx, sy) && !MON_AT(sx, sy) && !t_at(sx, sy)
                    ...
                    if ((sx + sy) % 2) {
                        ...
                        if (!eelct) {
                            ...
                            if (j == 1) {
                                (void) makemon(&mons[PM_GRINDYLOW], sx, sy, NO_MM_FLAGS);
                                eelct++;
                            }
                        }

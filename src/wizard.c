...
static NEARDATA const int nasties[] = {
    ...
    /* chaotic */
    PM_BLACK_DRAGON, PM_RED_DRAGON, PM_ARCH_LICH, PM_VAMPIRE_LEADER, PM_VAMPIRE_MAGE,
    PM_MASTER_MIND_FLAYER, PM_DISENCHANTER, PM_WINGED_GARGOYLE,
    PM_STORM_GIANT, PM_OLOG_HAI, PM_ELF_NOBLE, PM_ELVEN_MONARCH,
    PM_OGRE_TYRANT, PM_CAPTAIN, PM_GREMLIN, PM_TARASQUE,
    /* lawful */
...
static NEARDATA const unsigned wizapp[] = {
    PM_HUMAN,      PM_STORM_GIANT,  PM_VAMPIRE,       PM_RED_DRAGON,
...
strategy(struct monst *mtmp)
{
    ...
    if (!(is_covetous(mtmp->data) && !(In_hell(&u.uz) && !Is_sanctum(&u.uz) && noteleport_level(mtmp)))
    ...
    int factor = 3;
    if ((mtmp->data->mflags3 & M3_WANTSBOOK) && u.uhave.book)
        factor *= 2;
    else if ((mtmp->data->mflags3 & M3_WANTSBELL) && u.uhave.bell)
        factor *= 2;
    else if ((mtmp->data->mflags3 & M3_WANTSCAND) && u.uhave.menorah)
        factor *= 2;
    else if ((mtmp->data->mflags3 & M3_WANTSAMUL)
             && (u.uhave.book || u.uhave.bell || u.uhave.menorah || u.uhave.amulet))
        factor *= 2;
    else if (mtmp->data->mflags3 & M3_WANTSARTI) {
        for (struct obj *obj = gi.invent; obj; obj = obj->nobj)
    ...
    int switched = (mtmp->mhp * factor) / mtmp->mhpmax;
    if (factor == 6)
        if (switched >= 3 && switched <= 5)
            switched -= rn2(2) + rn2(2) + rn2(2);
    switch (switched) {
    ...
    int difcap) /* if non-zero, try to make difficulty be lower than this */
{
    ...
    /* if banished or too difficult or out of place, try a substitute
       when a suitable one exists
           arch-lich -> master lich,
           master mind flayer -> mind flayer,
       but the substitutes are likely to be banished too */
    alt = res;
    if ((gm.mvitals[res].mvflags & G_BANID) != 0
        || (difcap > 0 && mons[res].difficulty >= difcap)
         /* note: nasty() -> makemon() ignores G_HELL|G_NOHELL;
            ...
            outside of Gehennom (unless the latter has been banished) */
        || (mons[res].bani & (Inhell ? G_NOHELL : G_HELL)) != 0)
        alt = big_to_little(res);
    if (alt != res && (gm.mvitals[alt].mvflags & G_BANID) == 0) {
...
#define MAXNASTIES 10 /* more than this can be created */
    ...
    if (!rn2(10) && Inhell) {
        ...
        if (!summoner)
            ...
            tmp = (summoner->m_lev > 4) ? summoner->m_lev / 4 : 1;
        ...
        for (i = rnd(tmp); i > 0 && count < MAXNASTIES; --i) {
            ...
            for (j = 0; j < 20; j++) {
                ...
                if (summoner && !enexto(&bypos, summoner->mux, summoner->muy,
                                        &mons[makeindex]))
                    continue;
                /* this honors banishment but overrides extinction; it ignores
                ...
                if ((mtmp = makemon(&mons[makeindex], bypos.x, bypos.y,
                    ...
                    /* random monster to substitute for bani'd selection;
                ...
                if (mtmp) {
                    /* create at most one arch-lich or Archon regardless
                       ...
                       replacement for a banished selection) */
...
void
boom(struct monst *mtmp)
{
    if (Deaf)
        return;
    pline("%s lets out a defeaning cry.", Monnam(mtmp));
    wake_nearto(mtmp->mx, mtmp->my, 5 * 5);
}
/*wizard.c*/

...
mk_mplayer(struct permonst *ptr, coordxy x, coordxy y, boolean special)
{
    ...
    if ((mtmp = makemon(ptr, x, y, special ? (MM_NOMSG | NO_MINVENT): NO_MM_FLAGS)) != 0) {
        ...
        if (special) {
            ...
            mkmonmoney(mtmp, rn2(5000));
            quan = rn2(7);
            while (quan--) {
                (void) mpickobj(mtmp, mkgoodobj(RANDOM_CLASS, FALSE, TRUE));
            }
            if (rn2(2))  {
                otmp = pm_artifact(ptr);
                if (otmp)
                    (void) mpickobj(mtmp, otmp);
            }
        }

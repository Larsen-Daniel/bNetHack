...
Boots_on(void)
{
    ...
    if (haveboots) {
        if (objdescr_is(&boots, "combat boots")) {
            boots_bonus ++;
            u.uhitinc ++;
            u.udaminc ++;
            int discrepancy = 1 - boots_bonus;
            if (discrepancy > 1 || discrepancy < -1) { /* this code may run after the boots have been removed in the levitation case */
                pline("This was not supposed to happen... however, there are no ill effects.");
                u.uhitinc += discrepancy;
                u.udaminc += discrepancy;
            }
        }
    }
...
Boots_off(void)
{
    ...
    if (uarmf) {
        if (objdescr_is(uarmf, "combat boots")) {
            if (!gc.context.takeoff.cancelled_don) {
                boots_bonus --;
                u.uhitinc --;
                u.udaminc --;
                int discrepancy = 0 - boots_bonus;
                if (discrepancy > 1 || discrepancy < -1) {
                    pline("This was not supposed to happen... however, there are no ill effects.");
                    u.uhitinc += discrepancy;
                    u.udaminc += discrepancy;
                }
...
Cloak_on(void)
{
    ...
    if (uarmc) {
        if (objdescr_is(uarmc, "tattered cape")) {
            cloak_bonus --;
            ABON(A_CHA) --;
            int discrepancy = -1 - cloak_bonus;
            if (discrepancy > 1 || discrepancy < -1) {
                pline("This was not supposed to happen... however, there are no ill effects.");
                ABON(A_CHA) += discrepancy;
            }
            gc.context.botl = 1;
            // adj_abon(uarmc, -1, 0);
        }
        if (objdescr_is(uarmc, "ornamental cope") || objdescr_is(uarmc, "opera cloak")) {
            cloak_bonus ++;
            ABON(A_CHA) ++;
            int discrepancy = 1 - cloak_bonus;
            if (discrepancy > 1 || discrepancy < -1) {
...
Cloak_off(void)
{
    ...
    if (uarmc) {
        if (objdescr_is(otmp, "tattered cape")) {
            cloak_bonus ++;
            ABON(A_CHA) ++;
            int discrepancy = 0 - cloak_bonus;
            if (discrepancy > 1 || discrepancy < -1) {
                pline("This was not supposed to happen... however, there are no ill effects.");
                ABON(A_CHA) += discrepancy;
            }
        ...
        if (objdescr_is(otmp, "ornamental cope") || objdescr_is(otmp, "opera cloak")) {
            cloak_bonus --;
            ABON(A_CHA) --;
            int discrepancy = 0 - cloak_bonus;
            if (discrepancy > 1 || discrepancy < -1) {
...
Helmet_on(void)
{
    ...
    if (havehelm) {
        if (objdescr_is(&helm, "plumed helmet")) {
            helmet_bonus ++;
            ABON(A_CHA) ++;
            int discrepancy = 1 - helmet_bonus;
            if (discrepancy > 1 || discrepancy < -1) {
                pline("This was not supposed to happen... however, there are no ill effects.");
                ABON(A_CHA) += discrepancy;
            }
...
Helmet_off(void)
{
    ...
    if (uarmh) {
        if (objdescr_is(uarmh, "plumed helmet")) {
            if (!gc.context.takeoff.cancelled_don) {
                helmet_bonus --;
                ABON(A_CHA) --;
                int discrepancy = 0 - helmet_bonus;
                if (discrepancy > 1 || discrepancy < -1) {
                    pline("This was not supposed to happen... however, there are no ill effects.");
                    ABON(A_CHA) += discrepancy;
                }
    ...
    boolean on_purpose) /* voluntary removal; not applicable for putting on */
{
    ...
    case SHIMMERING_DRAGON_SCALE_MAIL:
        toggle_displacement(otmp, u.uprops[objects[otmp->otyp].oc_oprop].extrinsic & ~WORN_ARMOR, puton);
        break;
...
canwearobj(struct obj *otmp, long *mask, boolean noisy)
{
    ...
    if (Pinned) {
        if (noisy)
            You("can't put on any armor in your current shape.");
        return 0;
    }
...
dowear(void)
{
    ...
    if (Pinned) {
        pline("can't put on any armor in your current shape.");
        return ECMD_OK;
    }
...
void
adj_abon(register struct obj *otmp, register schar delta, boolean adjustment)
{
    int i;
    ...
    /* if (!adjustment) {
        if (uarmc && uarmc == otmp && objdescr_is(uarmc, "tattered cape")) {
            ABON(A_CHA) += (delta);
            gc.context.botl = 1;
        }
        if (uarmc && uarmc == otmp && (objdescr_is(uarmc, "ornamental cope") || objdescr_is(uarmc, "opera cloak"))) {
            ABON(A_CHA) += (delta);
            gc.context.botl = 1;
        }
        if (uarmh && uarmh == otmp && objdescr_is(uarmh, "plumed helmet")) {
            ABON(A_CHA) += (delta);
            gc.context.botl = 1;
        }
    } */
    if (is_art(otmp, ART_SCEPTRE_OF_MIGHT) && strength_type && sceptre_strength && Role_if(PM_CAVE_DWELLER)) {
        if (delta > 0)
            for (i = otmp->spe - delta; i <= otmp->spe - 1; i++)
                if (i >= 0)
                    ABON(A_STR)++;
        if (delta < 0)
            for (i = otmp->spe + 1; i <= otmp->spe - delta; i++)
                if (i >= 0)
                    ABON(A_STR)++;
        gc.context.botl = 1;
    }

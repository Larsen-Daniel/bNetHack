    ...
    unsigned cxn_flags) /* bitmask of CXN_xxx values */
{
    ...
    case AMULET_CLASS:
        ...
        break;
    case WEAPON_CLASS:
        if (is_poisonable(obj) && obj->opoisoned && !is_art(obj, ART_GRIMTOOTH))
            Strcpy(buf, "poisoned ");
        /*FALLTHRU*/
    case VENOM_CLASS:
    case TOOL_CLASS:
...
the_unique_pm(struct permonst* ptr)
{
    ...
    uniq = (ptr->bani & G_UNIQ) ? TRUE : FALSE;
    ...
    unsigned doname_flags) /* special case requests */
{
    ...
    case ARMOR_CLASS:
        ...
        /*FALLTHRU*/
    case WEAPON_CLASS:
        if (ispoisoned && !is_art(obj, ART_GRIMTOOTH))
        ...
        break;
    case TOOL_CLASS:
        /* if (((obj->oartifact == ART_ORB_OF_DETECTION && Role_if(PM_ARCHEOLOGIST)) || (obj->oartifact == ART_ORB_OF_FATE && Role_if(PM_VALKYRIE))) && obj->where == OBJ_INVENT)
            Strcat(bp, " (floating)"); */
 ...
 ring:
    ...
    if ((obj->owornmask & W_WEP) && !gm.mrg_to_wielded) {
        ...
        if ((obj->quan != 1L
            ...
            if (!Blind) {
                ...
                else if (gw.warn_obj_cnt && obj == uswapwep
    ...
    if (iflags.suppress_price || gp.program_state.restoring) {
        ...
        long unit_price = quotedprice / obj->quan;
        if (unit_price > 0L && obj->dknown && !obj->oartifact) {
            if (objects[obj->otyp].oc_minprice == -1 || unit_price < objects[obj->otyp].oc_minprice)
                objects[obj->otyp].oc_minprice = unit_price;
            if (objects[obj->otyp].oc_maxprice == -1 || unit_price > objects[obj->otyp].oc_maxprice)
                objects[obj->otyp].oc_maxprice = unit_price;
        }
        ...
        long unit_price = price / obj->quan;
        if (price > 0L) {
            if (unit_price > 0L && obj->dknown && !obj->oartifact) {
                if (objects[obj->otyp].oc_minprice == -1 || unit_price < objects[obj->otyp].oc_minprice)
                    objects[obj->otyp].oc_minprice = unit_price;
 ...
 * banishable, and vanquished-mon handling does its own special casing
 ...
 typfnd:
    ...
    if (d.mntmp >= LOW_PM) {
        ...
        case TIN:
            ...
            } else if ((!(mons[d.mntmp].bani & G_UNIQ) || wizard)
            ...
            break;
        case CORPSE:
            if ((!(mons[d.mntmp].bani & G_UNIQ) || wizard)
                && !(gm.mvitals[d.mntmp].mvflags & G_NOCORPSE)) {
    ...
    if(is_art(d.otmp, ART_GRIMTOOTH))
        d.otmp->opoisoned = 1;

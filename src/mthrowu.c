    ...
    const char *name) /* if null, then format `*objp' */
{
    ...
    int effectiveac = AC_VALUE(u.uac);
    if (effectiveac + tlev <= (dieroll = rnd(20))) {
        ++gm.mesg_given;
        if (Blind || !Verbose(2, thitu1)) {
            pline("It misses.");
        } else if (effectiveac + tlev <= dieroll - 2) {
            if (onm != onmbuf)
            ...
            default:
                {
                    ...
                    if (hitv < -4)
                        hitv = -4;
                    if (is_elf(mon->data)
                        && (objects[singleobj->otyp].oc_skill == P_BOW || objects[singleobj->otyp].oc_skill == -P_BOW)) {
                        int elfbonus = 0;
                        if (mon->data == &mons[PM_HIGH_ELF])
                            elfbonus = 3;
                        if (mon->data == &mons[PM_ELVEN_MONARCH])
                            elfbonus = 2;
                        if (mon->data == &mons[PM_ELF_NOBLE])
                            elfbonus = 1;
                        hitv += 2 * elfbonus + 1;
    ...
    if (m_lined_up(mtarg, mtmp)) {
        ...
        if (!mtmp->mspec_used && rn2(3)) {
            if (BZ_VALID_ADTYP(typ)) {
                ...
                if ((mtmp->data == &mons[PM_RED_DRAGON] || mtmp->data == &mons[PM_WHITE_DRAGON]) && !Is_waterlevel(&u.uz)) {
                    coord cc, movingcc;
                    int n;
                    cc.x = mtmp->mux;
                    cc.y = mtmp->muy;
                    n = rnd(8) + 1;
                    while (n--) {
                        if (mtmp->data == &mons[PM_WHITE_DRAGON])
                            explode(movingcc.x, movingcc.y,
                                SPE_CONE_OF_COLD - SPE_MAGIC_MISSILE + 10,
                                mtmp->m_lev, 0, EXPL_FROSTY);
                        else
                            explode(movingcc.x, movingcc.y,
                                SPE_FIREBALL- SPE_MAGIC_MISSILE + 10,
                                mtmp->m_lev, 0, EXPL_FIERY);
                        movingcc.x = cc.x + rnd(3) - 2;
                        movingcc.y = cc.y + rnd(3) - 2;

...
struct obj *
pm_artifact(struct permonst *ptr)
{
    int m = monsndx(ptr) - PM_ARCHEOLOGIST + ART_ORB_OF_DETECTION;
...
void
recompute_extrinsics(void)
{
    int i;
    for (i = 1; i <= LAST_PROP; i++) {
        u.uprops[i].extrinsic = u.uprops[i].extrinsic & ~W_WEP;
        u.uprops[i].extrinsic = u.uprops[i].extrinsic & ~W_SWAPWEP;
        // only for armor
        // u.upropos[i].blocked = u.upropos[i].blocked & ~W_WEP;
        // u.upropos[i].blocked = u.upropos[i].blocked & ~W_SWAPWEP;
    }
    if (uwep) {
        i = objects[uwep->otyp].oc_oprop;
        u.uprops[i].extrinsic = u.uprops[i].extrinsic | W_WEP;
        super_silent_mode = 1;
        set_artifact_intrinsic(uwep, 1, W_WEP);
        super_silent_mode = 0;
    }
    if (u.twoweap && uswapwep) {
        i = objects[uswapwep->otyp].oc_oprop;
        u.uprops[i].extrinsic = u.uprops[i].extrinsic | W_SWAPWEP;
        super_silent_mode = 1;
        set_artifact_intrinsic(uswapwep, 1, W_SWAPWEP);
        super_silent_mode = 0;
    }
}
void
set_artifact_intrinsic_extra(struct obj *otmp, boolean on, long wp_mask)
{
    if (!otmp)
        return;
    int p = objects[otmp->otyp].oc_oprop;
    if (!on)
        u.uprops[p].extrinsic = u.uprops[p].extrinsic & ~wp_mask;
    else
        u.uprops[p].extrinsic |= wp_mask;
...
set_artifact_intrinsic(struct obj *otmp, boolean on, long wp_mask)
{
    ...
    register uchar dtyp;
    register long spfx;
    if (!oart && otmp->otyp != AMULET_OF_YENDOR)
        return;
    if (!super_silent_mode) {
        /* if (wp_mask & W_WEAPONS)
            recompute = 1; */
        if (Role_if(PM_ARCHEOLOGIST) && !silent_archeologist_mode) {
            if (on && otmp->owornmask) {
            ...
            if (!on && carried(otmp)) {
                if (its_a_steal)
                    steal_powerful = 1;
                else if (much_less_powerful != 2) {
                    if (!much_less_powerful) {
                        if (last_powerful_message >= gm.moves - 10 && positive_message == -1)
    ...
    if (spfx & SPFX_CRSBLCK) {
        if (on)
            ECurse_blocking |= wp_mask;
        else
            ECurse_blocking &= ~wp_mask;
    }
    if (spfx & SPFX_REGEN) {
    ...
    if ((spfx & SPFX_REFLECT) && ((wp_mask & W_WEP) || (wp_mask & W_SWAPWEP))) {
        if (on)
            EReflecting |= wp_mask;
        else
            EReflecting &= ~wp_mask;
...
touch_artifact(struct obj *obj, struct monst *mon)
{
    ...
    if ((( badalign && self_willed) // (((badclass || badalign) && self_willed)
        || (badalign && (!yours || !rn2(4))))
        && (oart != &artilist[ART_HEART_OF_AHRIMAN] || uwep == obj)) {
        int dmg, tmp;
        char buf[BUFSZ];
    ...
    *dmgptr += rnd(4); /* (2..3)d4 */
    ...
    case MB_INDEX_SCARE:
        if (youdefend) {
            ...
            if (rn2(2) && smartresist(mdef, WEAPON_CLASS, 0, NOTELL))
                resisted = TRUE;
            else
    ...
    int dieroll)        /* needed for Magicbane and vorpal blades */
{
    ...
    *dmgptr += spec_dbon(otmp, mdef, *dmgptr);
    ...
    if (spec_ability(otmp, SPFX_BEHEAD)) {
        if (is_art(otmp, ART_TSURUGI_OF_MURAMASA)
            && (dieroll == 1 || (dieroll == 2 && ((youattack && Role_if(PM_SAMURAI)) || magr->data == &mons[PM_ASHIKAGA_TAKAUJI] || magr->data == &mons[PM_SAMURAI])))) {
            ...
            if (!youdefend) {
                ...
                *dmgptr = 2 * mdef->mhp + FATAL_DAMAGE_MODIFIER;
                ...
                if (bigmonst(gy.youmonst.data))
                    if (rn2(5) || !largemonst(gy.youmonst.data)) {
    ...
    if (oart->inv_prop > LAST_PROP) {
        ...
        case LIGHTNING_BLAST:
            if (!getdir((char *) 0)) {
                ...
                if (oart->inv_prop == FIRE_BLAST)
                    ubuzz(BZ_U_WAND(BZ_OFS_WAN(WAN_FIRE)), 6);
                if (oart->inv_prop == FROST_BLAST)
                    ubuzz(BZ_U_WAND(BZ_OFS_WAN(WAN_COLD)), 6);
                if (oart->inv_prop == LIGHTNING_BLAST)
                    ubuzz(BZ_U_WAND(BZ_OFS_WAN(WAN_LIGHTNING)), 6);
            }
            break;
        case CREATE_LIGHT:
            litroom(TRUE, obj);
        ...
        case CREATE_AMMO: {
            ...
            if (Role_if(PM_RANGER))
                ranger_bonus = 1;
            if (Role_if(PM_RANGER) && rn2(67) < 12) // same 55 : 12 ratio as for randomly generated projectiles
            ...
            for (mtmp = fmon; mtmp; mtmp = mtmp2) {
                ...
                if (chance <= 1 || !rn2(chance)) {
                    if (!Inhell) {
                        ...
                        u_teleport_mon(mtmp, FALSE, TRUE);
                    }
                } else {
                    nstayed++;
                }
        ...
        if (((eprop & ~W_ARTI) || iprop) && oart->inv_prop != CONFLICT) {
 nothing_special:

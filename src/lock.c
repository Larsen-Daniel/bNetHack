...
picklock(void)
{
    ...
    if (gx.xlock.usedtime++ >= 50 || !canusehands) {
...
forcelock(void)
{
    ...
    if (gx.xlock.usedtime++ >= 50 || !uwep || !canusehands) {
    ...
    struct obj *container)  /* container, for autounlock */
{
    ...
    if (gx.xlock.usedtime && picktyp == gx.xlock.picktyp) {
        ...
        if (nohands(gy.youmonst.data)) {
            ...
            pline(no_longer, "move your", makeplural(body_part(ARM)));
            reset_pick();
            return PICKLOCK_LEARNED_SOMETHING;
    ...
    if (!canusehands) {
        if (nohands(gy.youmonst.data))
            ...
            You_cant("use %s -- your %s are pinned.", doname(pick), makeplural(body_part(ARM)));
...
doopen_indir(coordxy x, coordxy y)
{
    ...
    if (!canusehands) {
        if (nohands(gy.youmonst.data))
            ...
            You_cant("open anything -- your %s are pinned.", makeplural(body_part(ARM)));
        return ECMD_OK;
    }
...
doclose(void)
{
    ...
    if (!canusehands) {
        if (nohands(gy.youmonst.data))
            ...
            You_cant("close anything -- your %s are pinned.", makeplural(body_part(ARM)));
        return ECMD_OK;
    }

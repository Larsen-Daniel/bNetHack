...
/* returns True if monster only has attacks using arms, False if it does */
boolean
onlyarmattacks(struct permonst* ptr)
{
    int i;
    struct attack *mattk = ptr->mattk;
    ...
    for (i = 0; i < NATTK; i++) {
        /* AT_BOOM "passive attack" (gas spore's explosion upon death)
           isn't an attack as far as our callers are concerned */
        if (mattk[i].aatyp == AT_BOOM || mattk[i].aatyp == AT_WEAP
            || mattk[i].aatyp == AT_CLAW || mattk[i].aatyp == AT_TUCH
            || mattk[i].aatyp == AT_MAGC || mattk[i].aatyp == AT_NONE)
            continue;
...
{
    /* non-stone golems turn into stone golems unless latter is banished */
    return (boolean) (is_golem(ptr) && ptr != &mons[PM_STONE_GOLEM]
                      && !(gm.mvitals[PM_STONE_GOLEM].mvflags & G_BANID));
    /* allow G_EXTINCT */
}

static unsigned short
relevant_resistance(int adtyp)
{
    switch (adtyp) {
    case AD_FIRE:
        return MR_FIRE;
...
defended(struct monst *mon, int adtyp)
{
    ...
    if (!is_you)
        mon->mwantedintrinsics |= relevant_resistance(adtyp);
...
resists_blnd(struct monst* mon)
{
    ...
    if (is_you ? (Blind || Unaware)
               : (mon->mblinded || !mon->mcansee || !haseyes(ptr)
                  ...
                  || mon->msleeping || (mon->mfrozen && mon->mfrozenr == REASON_SLEEPING)))
        return TRUE;
...
mstrength(struct permonst* ptr)
{
    ...
    for (i = 0; i < NATTK; i++) {
        ...
        if ((tmp2 == AD_DRLI) || (tmp2 == AD_STON) || (tmp2 == AD_DRST)
            || (tmp2 == AD_DRDX) || (tmp2 == AD_DRCO) || (tmp2 == AD_WERE) || (tmp2 == AD_MUMY))
            n += 2;
        else if (strcmp(ptr->pmnames[NEUTRAL], "grid bug"))
            n += (tmp2 != AD_PHYS);
        if ((int) (ptr->mattk[i].damd * ptr->mattk[i].damn) > 23) {
            n++;
            if (tmp < 2 && ptr->mattk[i].aatyp != AT_BOOM)
                n += 10;
        }
        if (tmp < 2 && (tmp2 == AD_FIRE || tmp2 == AD_DRST) && ptr->mattk[i].adtyp != AT_NONE)
            n += 4 * ((int) (ptr->mattk[i].damd * ptr->mattk[i].damn) > 5);
    }
    ...
    int *gender_name_var)
{
...

    {
        static const struct alt_spl names[] = {
            ...
            { "miengu", PM_JENGU, NEUTRAL},
            { "hali", PM_HALA, NEUTRAL},
            /* end of list */
            { 0, NON_PM, NEUTRAL }
        };
...
/* monster class from user input; used for banishment and controlled polymorph;
                        ...
                        * flags&2: random if hallucinating */
{
    ...
    if (is_neuter(mtmp->data))
        return 2;
    return (humanoid(mtmp->data) || (mtmp->data->bani & G_UNIQ)
...
resist_conflict(struct monst* mtmp)
{
    ...
    int resist_chance;
    int effective_lev = mtmp->m_lev;
    if (mtmp->data == &mons[PM_MARID])
        effective_lev /= 3;
    if (mtmp->data == &mons[PM_DJINNI])
        effective_lev *= 3;
    if (special_conflict == 0)
        resist_chance = min(19, (ACURR(A_CHA) - effective_lev + u.ulevel));
    else
        resist_chance = min(19, (ACURR(A_CHA) - effective_lev + u.ulevel + 5));
    if (u.uhave.amulet && rnd(2) == 1)
        return FALSE;

...
create_monster(monster *m, struct mkroom *croom)
{
    ...
    if (!class) {
        ...
        if ((pm->bani & G_UNIQ) && (g_mvflags & G_EXTINCT))
            return;
        if (g_mvflags & G_GONE) /* banished or extinct */
            pm = (struct permonst *) 0; /* make random monster */
    } else {
        pm = mkclass(class, G_NOGEN);
        /* if we can't get a specific monster type (pm == 0) then the
           class has been banished, so settle for a random monster */
    }
...
int
lspo_goodies(lua_State *L UNUSED)
{
	makegoodies();
	return 1;
}

...
artitouch(struct obj *obj)
{
    if (!Qstat(touched_artifact)) {
        ...
        qt_pager("gotit");
        if(Role_if(PM_ARCHEOLOGIST) || Role_if(PM_VALKYRIE))
        {
            if(!Blind)
                pline("The orb begins to glow with a warm light.");
            else
                pline("You feel the orb begin to heat up.");
            pline("Suddenly, it shoots out from your %s.",makeplural(body_part(HAND)));
            pline("Your load feels lighter as the orb begins to float on its own accord.");
            /*if(!Blind)
            {
                if(Role_if(PM_ARCHEOLOGIST))
                    pline("As you pick up the Orb of Detection, it begins to glow.");
                if(Role_if(PM_VALKYRIE))
                    pline("As you pick up the Orb of Fate, it begins to glow.");
                pline("Suddenly, you feel a weight lift as the orb floats above you.");
            }
            else
            {
                if(Role_if(PM_ARCHEOLOGIST))
                    pline("As you pick up the Orb of Detection, you feel it begin to heat up.");
                if(Role_if(PM_VALKYRIE))
                    pline("As you pick up the Orb of Fate, you feel it begin to heat up.");
                pline("Suddenly, you feel a weight lift as the orb floats up.");
            }*/
        }
        exercise(A_WIS, TRUE);
    }

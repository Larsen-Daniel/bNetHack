...
static void seffect_banishment(struct obj **);
...
static void do_class_banishment(void);
...
doread(void)
{
    ...
    if (otyp == FORTUNE_COOKIE) {
        ...
        const int red_mons[] = { PM_FIRE_ANT, PM_PYROLISK, PM_HELL_HOUND,
            ...
            PM_GIANT_BAT, PM_WATER_MOCCASIN, PM_FLESH_GOLEM, PM_PIRANHA };
...
maybe_tame(struct monst *mtmp, struct obj *sobj)
{
    ...
    if (sobj->cursed) {
        ...
        if (!smartresist(mtmp, sobj->oclass, 0, NOTELL) || mtmp->isshk)
...
seffect_enchant_armor(struct obj **sobjp)
{
    ...
    if (s) {
        ...
        if (s) /* skip if it got changed to 0 */
            adj_abon(otmp, s, 1); /* adjust armor bonus for Dex or Int+Wis */
        gk.known = otmp->known;
        /* update shop bill to reflect new higher price */
        if (s > 0 && otmp->unpaid)
            alter_cost(otmp, 0L);
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (cansee(mtmp->mx, mtmp->my)) {
            if (confused || scursed) {
                ...
                mtmp->mcanmove = 1;
            } else if (!smartresist(mtmp, sobj->oclass, 0, NOTELL))
...
static void
seffect_banishment(struct obj **sobjp)
{
    struct obj *sobj = *sobjp;
    int otyp = sobj->otyp;
    boolean sblessed = sobj->blessed;
    ...
    if (!already_known)
        You("have found a scroll of banishment!");
    gk.known = TRUE;
    if (sblessed)
        do_class_banishment();
    else
        do_banishment((!scursed) | (2 * !!Confusion));
}
...
do_class_banishment(void)
{
    ...
    for (j = 0;; j++) {
        ...
        Strcpy(promptbuf, "What class of monsters do you want to banish?");
        ...
        /* choosing "none" preserves banishless conduct */
        if (*buf == '\033' || !strcmpi(buf, "none")
            || !strcmpi(buf, "'none'") || !strcmpi(buf, "nothing")) {
            livelog_printf(LL_BANISHMENT,
                           "declined to perform class banishment");
            return;
        }
        ...
        for (i = LOW_PM; i < NUMMONS; i++) {
            if (mons[i].mlet == class) {
                if (!(mons[i].bani & G_BANI))
                    immunecnt++;
                else if (gm.mvitals[i].mvflags & G_BANID)
                    gonecnt++;
                else
                    goodcnt++;
            }
        ...
        if (!goodcnt && class != mons[gu.urole.mnum].mlet
            ...
            if (gonecnt)
                ...
                You("aren't permitted to banish such monsters.");
        ...
        for (i = LOW_PM; i < NUMMONS; i++) {
            if (mons[i].mlet == class) {
                 ...
                 * from both race and role; thus banishment affects either.
                 */
                if (Your_Own_Role(i) || Your_Own_Race(i)
                    || ((mons[i].bani & G_BANI)
                        && !(gm.mvitals[i].mvflags & G_BANID))) {
                    /* This check must be first since player monsters might
                     * have G_BANID or !G_BANI.
                     */
                    if (!ll_done++) {
                        if (!num_banishments())
                            livelog_printf(LL_CONDUCT | LL_BANISHMENT,
                                     "performed %s first banishment (class %c)",
                                           uhis(), def_monsyms[class].sym);
                        else
                            livelog_printf(LL_BANISHMENT, "banished class %c",
                                           def_monsyms[class].sym);
                    }

                    gm.mvitals[i].mvflags |= (G_BANID | G_NOCORPSE);
                    ...
                    if (Upolyd && i == u.umonnum) {
                        ...
                        if (Unchanging) {
                            ...
                            /* finish banishing this class of
                               monsters before ultimately dying */
                            gameover = TRUE;
                        } else
                            rehumanize();
                    }
                    /* Self-banishment if it matches either your race
                    ...
                    if ((mons[i].msound != MS_LEADER
                        ...
                        uniq = (mons[i].bani & G_UNIQ) ? TRUE : FALSE;
                        /* one special case */
                        if (i == PM_HIGH_CLERIC)
                            uniq = FALSE;

                        You("aren't permitted to banish %s%s.",
                            (uniq && !named) ? "the " : "",
        ...
        if (gameover || u.uhp == -1) {
            ...
            Strcpy(gk.killer.name, "scroll of banishment");
            if (gameover)
                done(BANISHED);
        }
        return;
...
void
do_banishment(
    int how) /* 0 = no banishment; create monsters (cursed scroll)
              * 1 = normal banishment
              * 3 = forced banishment of player
              * 5 (4 | 1) = normal banishment from throne */
{
    char buf[BUFSZ], promptbuf[QBUFSZ];
    int i, killplayer = 0;
    int mndx;
    ...
    if (how & PLAYER) {
        ...
        for (i = 0; ; i++) {
            ...
            Strcpy(promptbuf, "What type of monster do you want to banish?");
            ...
            if (*buf == '\033' || !strcmpi(buf, "none")
                ...
                livelog_printf(LL_BANISHMENT, "declined to perform banishment");
                return;
            }

            mndx = name_to_mon(buf, (int *) 0);
            if (mndx == NON_PM || (gm.mvitals[mndx].mvflags & G_BANID)) {
    ...
    if (Hallucination) {
        ...
        if ((ptr->bani & G_UNIQ) && ptr != &mons[PM_HIGH_CLERIC])
            which = !type_is_pname(ptr) ? "the " : "";
    }
    if (how & REALLY) {
        if (!num_banishments())
            livelog_printf(LL_CONDUCT | LL_BANISHMENT,
                           "performed %s first banishment (%s)",
                           uhis(), makeplural(buf));
        else
            livelog_printf(LL_BANISHMENT, "banished %s", makeplural(buf));

        /* setting no-corpse affects wishing and random tin generation */
        gm.mvitals[mndx].mvflags |= (G_BANID | G_NOCORPSE);
        ...
        if (killplayer) {
            ...
            if (how & PLAYER) {
                ...
                Strcpy(gk.killer.name, "banishing confusion");
        ...
        if (!(mons[mndx].bani & G_UNIQ)
            && !(gm.mvitals[mndx].mvflags & (G_BANID | G_EXTINCT)))
            for (i = rn1(3, 4); i > 0; i--) {

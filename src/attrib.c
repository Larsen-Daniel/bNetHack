    ...
    boolean thrown_weapon) /* thrown weapons are less deadly */
{
    ...
    if (EPoison_resistance || (HPoison_resistance && rn2(5))
            || is_innate_to_current_form(&HPoison_resistance)) {
        if (blast)
            shieldeff(u.ux, u.uy);
    ...
    if (i >= LOW_PM && (mons[i].bani & G_UNIQ)) {
        kprefix = KILLED_BY;
    ...
    if (i == 0 && typ != A_CHA && !Poison_resistance) {
    ...
    } else if (i * 2 > fatal || (!Poison_resistance && rn2(20))) {
        ...
        if ((blast || cloud) && Half_gas_damage) /* worn towel */
            loss = (loss + 1) / 2;
        loss = resist_damage(AD_DRST, loss);
        losehp(loss, pkiller, kprefix); /* poison damage */
    } else {
...
innately(long *ability)
{
    ...
    if ((iptr = check_innate_abil(ability, FROMRACE)) != 0)
        return FROM_RACE;
    /* the above two are the most fundamental causes */
    if ((*ability & FROMFORM) != 0L && Upolyd)
        return FROM_FORM;
    if ((iptr = check_innate_abil(ability, FROMEXPER)) != 0)
        return (iptr->ulevel == 1) ? FROM_ROLE : FROM_EXP;
    /* resistances from a poly form provide more protection than from experience,
       hence listed first */
...
boolean
is_innate_to_current_form(long *ability)
{
    int innateness = innately(ability);
    if (innateness == FROM_ROLE || innateness == FROM_RACE || innateness == FROM_FORM)
        return 1;
    return 0;
}
...
acurr(int x)
{
    ...
    if (x == A_STR) {
        if ((!strength_type && (tmp >= 125 || (uarmg && uarmg->otyp == GAUNTLETS_OF_POWER))) || (strength_type && (tmp >= 25 || (uarmg && uarmg->otyp == GAUNTLETS_OF_POWER)))) {
            if (!strength_type)
                return (schar) 125;
            else
                return (schar) 25;
        ...
        if (tmp < 18
            && gy.youmonst.data->mlet == S_NYMPH)
                // #if 0 || u.umonnum == PM_AMOROUS_DEMON)) #endif
            return (schar) 18;
    } else if (x == A_CON) {
        if (u_wield_art(ART_OGRESMASHER))
            return (schar) 25 - perm_con_damage;

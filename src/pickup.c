...
in_container(struct obj *obj)
{
    ...
    if (!gc.current_container) {
        ...
        return 0;
    } else if ((obj->oartifact == ART_ORB_OF_DETECTION && Role_if(PM_ARCHEOLOGIST)) || (obj->oartifact == ART_ORB_OF_FATE && Role_if(PM_VALKYRIE))) {
        pline("You reach out to grab %s.", the(xname(obj)));
        pline("Before you can stow the orb away, it wrests itself from your grasp.");
        return 0;

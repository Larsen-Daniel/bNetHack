...
auto_throw(void)
{
    ...
    if (!mon)
        impossible("no monster targeted by the Longbow of Diana?");
    gb.bhitpos.x = mon->mx;
    gb.bhitpos.y = mon->my;
    if (Role_if(PM_RANGER))
        ranger_bonus = 1;
    if (mon_hates_silver(mon) && Role_if(PM_RANGER) && rn2(67) < 12) // same 55 : 12 ratio as for randomly generated projectiles
        obj = mksobj(SILVER_ARROW, TRUE, FALSE);
    else
        obj = mksobj(ARROW, TRUE, FALSE);
    if (Role_if(PM_RANGER))
        ranger_bonus = 0;
    obj->cursed = uwep->cursed;
    obj->blessed = uwep->blessed;
    ...
    gm.m_shot.s = TRUE;
    // pline("You shoot %d glowing arrow%s as %s appear%s on the Longbow of Diana.", multishot, (multishot == 1) ? "" : "s", (multishot == 1) ? "it" : "they", (multishot == 1) ? "s" : "");
    // buffer_len = 0;
    // this would be handled later in the older scheme
    if (multishot == 1)
        pline("A glowing arrow emerges from the Longbow of Diana.");
    else
        pline("%d glowing arrows emerge from the Longbow of Diana.", multishot);
    gm.m_shot.o = obj->otyp;
    gm.m_shot.n = multishot;
    obj->quan = 1;
    int tmp, disttmp;
    int hmode = HMON_THROWN;
    int dieroll;
    int mdied = 0;
    boolean guaranteed_hit = engulfing_u(mon);
    actual_arrow_count = 0;
            ...
            */
            ...
            if (tmp >= dieroll) {
                ...
                if (hmon(mon, obj, hmode, dieroll)) { /* mon still alive */
                    if (mon->wormno)
                        cutworm(mon, gb.bhitpos.x, gb.bhitpos.y, 0);
                }
                else
                    mdied = 1;
                exercise(A_DEX, TRUE);
            ...
            impossible("no monster targeted by the Longbow of Diana within loop?"); // this could happen if the monster was still dead, but we checked for that
    }
    // buffer_print_please = 1;
    // clearbuffer();
    if (actual_arrow_count < multishot) {
        if (actual_arrow_count == multishot - 1)
            pline("The remaining arrow disappears midair.");
        else
            pline("The %d remaining arrows disappear midair.",multishot - actual_arrow_count);
    }
    actual_arrow_count = 0;
...
throw_obj(struct obj *obj, int shotlimit)
{
    ...
    if (!strength_type) {
        ...
        if ((is_art(obj, ART_MJOLLNIR) && ACURR(A_STR) < 25)
        || (obj->otyp == BOULDER && !throws_rocks(gy.youmonst.data))) {
        pline("It's too heavy.");
        res = ECMD_TIME;
        goto unsplit_stack;
...
ok_to_throw(int *shotlimit_p) /* (see dothrow()) */
{
    *shotlimit_p = LIMIT_TO_RANGE_INT(0, LARGEST_INT, gc.command_count);
    ...
    if (notake(gy.youmonst.data)) {
        ...
        You_cant("throw or shoot without using your %s.", makeplural(body_part(ARM))); /* not body_part(HAND) */
        return FALSE;
    }
...
throw_ok(struct obj *obj)
{
    ...
    if (!strength_type) {
        ...
        if (AutoReturn(obj, obj->owornmask)
            /* to get here, obj is boomerang or is uwep and (alkys or Mjollnir) */
            && (obj->oartifact != ART_MJOLLNIR || ACURR(A_STR) >= 25))
            return GETOBJ_SUGGEST;
...
dofire(void)
{
    ...
    boolean uwep_Throw_and_Return;
    if (!strength_type)
        uwep_Throw_and_Return = (uwep && AutoReturn(uwep, uwep->owornmask)
        ...
        if (dmg > 0) {
            ...
            if (Role_if(PM_ARCHEOLOGIST)) {
                ...
                if (touching_artifacts)
                    dmg += rn2(2 * touching_artifacts) + rn2(2 * touching_artifacts);
            }
        }
        if (dmg < 0)
...
int mjollnir_alignment_score(void);
int
mjollnir_alignment_score(void)
{
    if (u.ualign.record < 0)
        return 0;
    if (u.ualign.record > 20)
        return 20;
    return u.ualign.record;
}
    ...
    struct obj *oldslot) /* for thrown-and-return used with !fixinv */
{
 ...
 swallowit:
        ...
        if (iflags.returning_missile) { /* Mjollnir or aklys */
            if (rn2(200) >= 100 - 5 * mjollnir_alignment_score()) {
                if (tethered_weapon)
                    tmp_at(DISP_END, BACKTRACK);
                else
                    sho_obj_return_to_u(obj); /* display its flight */

                if (!impaired && rn2(200) >= 100 - 5 * mjollnir_alignment_score()) {

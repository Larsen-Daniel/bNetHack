...
fill_ordinary_room(struct mkroom *croom)
{
     ...
     *  20% chance for at least 1 box, regardless of number
     *  of rooms; about 2.5 - 3.75% for 2 boxes, least likely
     *  when few rooms; chance for 3 or more is negligible.
     */
    if (!rn2(gn.nroom * 10 / 2) && somexyspace(croom, &pos))
 ...
 skip_nonrogue:
    if (!rn2(15) && somexyspace(croom, &pos)) {
...
static void
dealwithstorerooms(void)
{
    int x, y, xx, yy, notstoreroom, nstorerooms = 0, artifactplaced = 0;
    int objecttypes[4] = {ARMOR_CLASS, WEAPON_CLASS, GEM_CLASS, FOOD_CLASS};
    int sortedobjecttypes[4] = {-1, -1, -1, -1};
    for (x = 0; x < 4; x++) {
        for(;;) {
            y = rn2(4);
            for (xx = 0; xx < x; xx++)
                if (sortedobjecttypes[xx] == objecttypes[y])
                    break;
            if (xx == x)
                break;
        }
        sortedobjecttypes[x] = objecttypes[y];
    }
    for (x = 10; x < COLNO - 10; x++)
        for (y = 5; y < ROWNO - 5; y++) {
            ...
            for (xx = 0; xx < 7; xx++)
                for (yy = 0; yy < 2; yy++)
                    if (levl[x + xx][y + yy].typ != ROOM)
                        notstoreroom = 1;
            if (levl[x - 1][y].typ == ROOM)
                notstoreroom = 1;
            if (levl[x + 7][y].typ == ROOM)
                notstoreroom = 1;
            if (levl[x][y - 1].typ == ROOM)
                notstoreroom = 1;
            if (levl[x][y + 2].typ == ROOM)
                notstoreroom = 1;
            if (notstoreroom)
                continue;
            for (xx = 0; xx < 7; xx++)
                for (yy = 0; yy < 2; yy++)
                    if (!artifactplaced || !rn2(10)) {
                        if (mkgoodobj_at(sortedobjecttypes[nstorerooms], x + xx, y + yy, TRUE, FALSE)->oartifact)
                            artifactplaced = 1;
                    }
                    else
                        mkgoodobj_at(sortedobjecttypes[nstorerooms], x + xx, y + yy, FALSE, FALSE);
            nstorerooms++;
        }
}

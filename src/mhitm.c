...
mattackm(register struct monst *magr, register struct monst *mdef)
{
    ...
    tmp = find_mac(mdef) + adjust_level(magr->m_lev);
    ...
    if (mdef->mundetected) {
        ...
        if (canseemon(mdef) && !sensemon(mdef)) {
            if (Unaware) {
                boolean justone = (mdef->data->bani & G_UNIQ) != 0L;
                const char *montype;
    ...
    for (i = 0; i < NATTK; i++) {
        ...
        case AT_WEAP: /* "hand to hand" attacks */
            if (distmin(magr->mx, magr->my, mdef->mx, mdef->my) > 1 || (is_elf(pa) && (mwep = select_rwep(magr)) && MON_WEP(magr) && ammo_and_launcher(mwep, MON_WEP(magr)) && mwep->quan >= 5)) {
...
gazemm(struct monst *magr, struct monst *mdef, struct attack *mattk)
{
    ...
    if (magr->data == &mons[PM_MEDUSA] && mon_reflects(mdef, (char *) 0)) {
        ...
        if (mdef->mcansee) {
            ...
            if (canseemon(magr))
                pline("%s seems unphased by her reflection.", Monnam(magr));
            return MM_MISS;
            /* monstone(magr);
            if (!DEADMONSTER(magr))
                return MM_MISS;

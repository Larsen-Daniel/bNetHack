...
static NEARDATA const char *deaths[] = {
    ...
    "turned into slime", "banished", "panic", "trickery", "quit",
    "escaped", "ascended"
};
...
static NEARDATA const char *ends[] = {
    ...
    "turned into slime", "were banished",
...
done_in_by(struct monst *mtmp, int how)
{
    ...
    if ((mptr->bani & G_UNIQ) != 0 && !(imitator && !mimicker)
    ...
    /* this could happen if a high-end vampire kills the hero
       when ordinary vampires are banished; ditto for wraiths */
    if (u.ugrave_arise >= LOW_PM
        && (gm.mvitals[u.ugrave_arise].mvflags & G_BANID))
...
#define FERVENT 9 // same as in pray.c
...
done(int how)
{
    ...
    if (how == ASCENDED || (!gk.killer.name[0] && how == BANISHED))
    ...
    if (Lifesaved && (how <= BANISHED)) {
        ...
        savelife(how);
        if (how == BANISHED) {
            pline("Unfortunately you are still banished...");
        } else {
            char killbuf[BUFSZ];
    ...
    if (!survive && !Inhell && can_pray(FALSE) && how < BANISHED && u.ualign.record >= FERVENT && never_saved_before) {
        if (is_undead(gy.youmonst.data) && (u.ualign.type == A_LAWFUL || (u.ualign.type == A_NEUTRAL && !rn2(10))))
            ;
        else {
            never_saved_before = 0;
            pline("But wait...");
            pline("%s takes pity on you.",align_gname(u.ualign.type));
            if (how == CHOKING)
                You("vomit ...");
            You_feel("much better!");
            // (void) adjattrib(A_CON, -1, TRUE);
            ...
            survive = TRUE;
            u.ublesscnt = rnz(350);
            int kick_on_butt = u.uevent.udemigod ? 1 : 0;
    ...
    if (!survive && (wizard || discover) && how <= BANISHED
...
really_done(int how)
{
    ...
    bones_ok = (how < BANISHED) && can_make_bones();
    ...
    if (how == PANICKED)
        ...
        u.ugrave_arise = (NON_PM - 1); /* statue instead of corpse */
             ...
             /* it's possible to turn into slime even though green slimes
                have been banished:  banishment could occur after hero is
                already infected or hero could eat a glob of one created
                before banishment; don't try to arise as one if they're gone */
             && !(gm.mvitals[PM_GREEN_SLIME].mvflags & G_BANID))
    ...
    if (have_windows) {
        ...
        if (how < BANISHED && flags.tombstone && endwin != WIN_ERR)
            outrip(endwin, how, endtime);
    } else
        done_stopprint = 1; /* just avoid any more output */

#ifdef DUMPLOG
    /* 'how' reasons beyond banishment shouldn't show tombstone;
       for normal end of game, banishment doesn't either */
    if (how <= BANISHED) {
        dump_redirect(TRUE);
        if (iflags.in_dumplog)

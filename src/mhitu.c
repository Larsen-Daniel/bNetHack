...
mattacku(register struct monst *mtmp)
{
    ...
    if (u.uundetected && !range2 && foundyou && !u.uswallow) {
        ...
        if (is_hider(gy.youmonst.data) && u.umonnum != PM_TRAPPER) {
            ...
            if (obj && is_metallic(obj)) {
                ...
                if (3 + find_mac(mtmp) <= rndmaybedoublehigh(20)) { // this is confusing because we want this to hit, but in this case, that means we want this random number to be high
    ...
    if (!range2 && (mtmp->cham == PM_NUCKELAVEE || mons[mtmp->cham].mlet == S_VAMPIRE)) {
        newcham(mtmp, &mons[mtmp->cham], NC_SHOW_MSG);
    }
    int weaponattack = 0;
    for (i = 0; i < NATTK; i++) {
        ...
        case AT_TENT:
            if (!range2 && (!MON_WEP(mtmp) || mtmp->mconf || Conflict
                ...
                if (foundyou) {
                    if (tmp > (j = rndmaybedoublehigh(20 + i))) {
        ...
        case AT_ENGL:
            if (!range2) {
                if (foundyou) {
                    if (u.uswallow
                        || (!mtmp->mspec_used && tmp > (j = rndmaybedoublehigh(20 + i)))) {
        ...
        case AT_SPIT:
            ...
            break;
        case AT_WEAP:
            if (range2 || (!Is_rogue_level(&u.uz) && is_elf(mdat) && (mon_currwep = select_rwep(mtmp)) && MON_WEP(mtmp) && ammo_and_launcher(mon_currwep, MON_WEP(mtmp)) && mon_currwep->quan >= 5)) {
                if (!weaponattack && !Is_rogue_level(&u.uz))
                    thrwmu(mtmp);
            } else {
                int hittmp = 0;
                ...
                if (foundyou) {
                    ...
                    if (tmp > (j = gm.mhitu_dieroll = rndmaybedoublehigh(20 + i)))
                        sum[i] = hitmu(mtmp, mattk);
                    else
                        missmu(mtmp, (tmp == j), mattk);
...
summonmu(struct monst *mtmp, boolean youseeit)
{
    ...
    if (is_demon(mdat) && (mdat->bani & G_NOCORPSE)) { /* not having a corpse is a proxy for being magical */
        if (mdat != &mons[PM_BALROG]) { // #if 0 && mdat != &mons[PM_AMOROUS_DEMON]) { #endif
            if (!rn2(13))
                (void) msummon(mtmp);
        }
...
int
armor_reduction(int dmg)
{
    if (dmg && u.uac < 0) {
        int i, j, damage_reduction, random_armor;
        i = dmg / 3;
        j = -u.uac;
        if (j > i)
            damage_reduction = i;
            ...
            damage_reduction = j;
        random_armor = j - damage_reduction;
        dmg -= damage_reduction;
        if (random_armor > 0)
            dmg -= rnd(random_armor);
        if (dmg < 1)
            dmg = 1;
        /* i = rn1(60, 15);
        if (dmg > i) {
...
hitmu(register struct monst *mtmp, register struct attack *mattk)
{
     ...
     *  against hits.
     */
    mhm.damage = armor_reduction(mhm.damage);
...
gulpmu(struct monst *mtmp, struct attack *mattk)
{
    ...
    case AD_ACID:
        if (Acid_resistance) {
            You("are covered with a mostly harmless goo.");
            monstseesu(M_SEEN_ACID);
            tmp = resist_damage(AD_ACID, tmp);
        } else {
            if (Hallucination)
...
gazemu(struct monst *mtmp, struct attack *mattk)
{
    ...
    case AD_STON:
        ...
        if (reflectable) {
            ...
            if (useeit)
                pline("%s seems unphased by her reflection.", Monnam(mtmp));
            break;
            /* gs.stoned = TRUE;
            killed(mtmp);
    ...
    case AD_FIRE:
        if (mcanseeu && !mtmp->mspec_used && rn2(5)) {
            if (cancelled) {
                ...
                if (Fire_resistance) {
                    shieldeff(u.ux, u.uy);
                    pline_The("fire doesn't feel that hot.");
                    monstseesu(M_SEEN_FIRE);
    ...
    struct attack *mattk) /* non-Null: current atk; Null: general capability */
{
    ...
    /* nymphs have two attacks, one for steal-item damage and the other
       ...
       don't pass the test */
    if ((pagr->mlet != S_NYMPH) // #if 0 && pagr != &mons[PM_AMOROUS_DEMON] #endif
...
doseduce(struct monst *mon)
{
    ...
    boolean fem = (Mgender(mon) == FEMALE); /* otherwise incubus */
    // #if 0 mon->data == &mons[PM_AMOROUS_DEMON] #endif

...
{
    /* demon court in Gehennom only ones able to teleport */
    if (In_hell(&u.uz) && !Is_sanctum(&u.uz) && !(is_dlord(mon->data) || is_dprince(mon->data)))
...
boolean
u_teleport_mon(struct monst* mtmp, boolean give_feedback, boolean byforce)
{
    coord cc;
    if (!byforce)
        if (In_hell(&u.uz) && !Is_sanctum(&u.uz) && tele_restrict(mtmp))
            return FALSE;

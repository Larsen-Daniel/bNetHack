...
static int
rndmaybedouble(int x)
{
    if (!Serendipitous && !Unserendipitous)
        return rnd(x);
    int i = rnd(x);
    int j = rnd(x);
    if (Serendipitous) {
    ...
    struct obj *wep)    /* uwep for do_attack(), null for kick_monster() */
{
    ...
    if (flags.confirm && mtmp->mpeaceful
        ...
        /* Intelligent chaotic weapons (Stormbringer) want blood no longer */
...
do_attack(struct monst *mtmp)
{
    ...
    if (is_safemon(mtmp) && !gc.context.forcefight) {
        if (TRUE) { // }!u_wield_art(ART_STORMBRINGER)) {
    ...
    if (!Upolyd && Pinned) {
        You("cannot attack with pinned %s.", makeplural(body_part(ARM)));
        return TRUE;
    }
    ...
    if (Upolyd && Pinned && onlyarmattacks(gy.youmonst.data)) {
        You("have no way to attack monsters physically right now.");
        mtmp->mstrategy &= ~STRAT_WAITMASK;
        goto atk_done;
    }
    ...
    if(is_art(uwep, ART_LONGBOW_OF_DIANA)) {
        auto_throw();
        return TRUE;
    }
    if (gu.unweapon) {
    ...
    if (!obj) { /* attack with bare hands */
        ...
        if (obj->oclass == WEAPON_CLASS || is_weptool(obj)
            ...
            if (/* if you strike with a bow... */
                ...
                if (is_art(obj, ART_GRIMTOOTH) && obj->opoisoned && is_poisonable(obj)) // Grimtooth
                    ispoisoned = TRUE;
            }
...
#undef useup_eggs
                ...
                default:
                    ...
                    if (is_wet_towel(obj)) {
                        boolean doubld = (mon->data == &mons[PM_IRON_GOLEM] || mon->data == &mons[PM_STEEL_GOLEM]);
    ...
    if (ispoisoned) {
        ...
        if (obj && !rn2(nopoison) && actual_arrow_count == 0 && !is_art(obj, ART_GRIMTOOTH)) { // this is ugly, but it works
    ...
    if (poiskilled) {
        ...
        nohandglow(mon);
        if (!mon->mconf && !smartresist(mon, SPBOOK_CLASS, 0, NOTELL)) {
            mon->mconf = 1;
...
shade_aware(struct obj *obj)
{
    ...
    if (obj->otyp == BOULDER
        ...
        || obj->otyp == IRON_CHAIN
        || obj->otyp == STEEL_CHAIN      /* dmgval handles those first four */
...
demonpet(void)
{
    ...
    i = !rn2(6) ? ndemon(u.ualign.type, (struct permonst *) 0) : NON_PM;
...
dosteal(void) // rogue ability
{
    ...
    if (mtmp->mx != tx || mtmp->my != ty)
        return ECMD_TIME; /* don't show minvent for long worm tail */

    if (mtmp->minvent) {
        probe_objchain(mtmp->minvent);
        pline("Select an object to steal.");
        otmp = display_minventory(mtmp, MINV_ALL | PICK_ONE,
                                  (char *) 0);
        if (!otmp)
            return ECMD_CANCEL;
        if (((otmp->owornmask & W_ARM) || (otmp->owornmask & W_ARMU)) && which_armor(mtmp, W_ARMC) != 0) {
            pline("You try to steal %s, but it is blocked by the %s.", the(xname(otmp)), cloak_simple_name(which_armor(mtmp, W_ARMC)));
            return ECMD_TIME;
        }
        if ((otmp->owornmask & W_ARMU) && which_armor(mtmp, W_ARM) != 0) {
            pline("You try to steal %s, but it is blocked by the %s.", the(xname(otmp)), suit_simple_name(which_armor(mtmp, W_ARM)));
            return ECMD_TIME;
        }
        if (otmp->owornmask && !Role_if(PM_ROGUE)) {
            pline("You try to steal %s but can't wrest it off %s.",the(xname(otmp)),noit_mon_nam(mtmp));
            return ECMD_TIME;
        }
        ...
        pline("%s has nothing for you to steal.",noit_Monnam(mtmp));
    }
    return ECMD_TIME;
}
static void
              ...
              struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (rustsconsiderably(pd)) { /* steel golem */
            pline("%s rusts.", Monnam(mdef));
            mhm->damage = rn1(25, 25);
        }
        else
              ...
              struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        /* uhitm */
        if (!negated && rn2(u.ulevel + 2) >= 6)
            xdrainenergym(mdef, TRUE);
        mhm->damage = 0;
    } else if (mdef == &gy.youmonst) {
        /* mhitu */
        hitmsg(magr, mattk);
        if (!negated && rn2(magr->m_lev + 2) >= 6)
            drain_en(mhm->damage);
        mhm->damage = 0;
    } else {
        /* mhitm */
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated && !rn2(3)
            ...
            if (Upolyd)
                uattklev = mons[u.umonnum].mlevel;
            else
                uattklev = u.ulevel;
            if (magr->data->mlet == S_VAMPIRE && mattk->aatyp != AT_WEAP)
                mhm->damage = 1 + (boolean) ( rn2(uattklev) >= 10) + (boolean) ( rn2(uattklev) >= 20);
            else
        ...
        if (is_death
            ...
            if (!is_death) {
                if (magr->data->mlet == S_VAMPIRE && mattk->aatyp != AT_WEAP)
                    mhm->damage = 1 + (boolean) ( rn2(magr->m_lev) >= 10) + (boolean) ( rn2(magr->m_lev) >= 20);
                else
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        hitmsg(magr, mattk);
        if (!negated) {
            pline("You're %s%c", on_fire(pd, mattk), (!completelyburns(pd) && Fire_resistance) ? '.' : '!');
            if (completelyburns(pd)) { /* paper or straw golem */
                ...
                mhm->damage = resist_damage(AD_FIRE, mhm->damage);
            }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        hitmsg(magr, mattk);
        if (!negated) {
            pline("You're covered in frost%c", Cold_resistance ? '.' : '!');
            if (Cold_resistance) {
                monstseesu(M_SEEN_COLD);
                mhm->damage = resist_damage(AD_COLD, mhm->damage);
            }
            if ((int) magr->m_lev > rn2(20))
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        hitmsg(magr, mattk);
        if (!negated) {
            You("get zapped%c", Shock_resistance ? '.' : '!');
            if (Shock_resistance) {
                monstseesu(M_SEEN_ELEC);
                mhm->damage = resist_damage(AD_ELEC, mhm->damage);
            }
            if ((int) magr->m_lev > rn2(20))
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && !rn2(3))
            if (Acid_resistance) {
                pline("You're covered in %s, but it seems mostly harmless.",
                      hliquid("acid"));
                monstseesu(M_SEEN_ACID);
                mhm->damage = resist_damage(AD_ACID, mhm->damage);
            } else {
              ...
              struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated) {
            ...
            if (u_teleport_mon(mdef, FALSE, TRUE) && u_saw_mon
...
void
mhitm_ad_mumy(struct monst *magr, struct attack *mattk UNUSED, struct monst *mdef,
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (uarmh && (rn2(8) || is_art(uarmh, ART_MITRE_OF_HOLINESS))) {
            /* not body_part(HEAD) */
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    boolean grabber = (pa == &mons[PM_ORANGE_DRAGON] || pa == &mons[PM_GRINDYLOW] || pa == &mons[PM_TARASQUE]);
    boolean large_defender;
    if (mdef == &gy.youmonst) {
        if (Upolyd) {
            large_defender = bigmonst(&mons[u.umonnum]);
        }
        else
            large_defender = 0;
    }
    else {
        large_defender = bigmonst(pd);
    }
    boolean ineffective_grindylow = (pa == &mons[PM_GRINDYLOW] && large_defender);

    if (magr == &gy.youmonst) {
        /* uhitm */
        if (!sticks(pd) && !ineffective_grindylow) {
            if (!u.ustuck && !rn2(grabber ? 3 : 10)) {
                if (m_slips_free(mdef, mattk)) {
                    mhm->damage = 0;
                } else {
                    if (grabber)
                        You("grab %s!", mon_nam(mdef));
                    else
        ...
        /* mhitu */
        if ((!magr->mcan || u.ustuck == magr) && !sticks(pd) && !ineffective_grindylow) {
            if (!u.ustuck && !rn2(grabber ? 3 : 10)) {
                if (u_slip_free(magr, mattk)) {
                    mhm->damage = 0;
                } else {
                    ...
                    if (grabber)
                        urgent_pline("%s grabs you!", Some_Monnam(magr));
                    else
        ...
        /* mhitm */
        if (magr->mcan || !ineffective_grindylow)
            mhm->damage = 0;
    }
}
        ...
        if (!rn2(4) && !slimeproof(pd)) {
            if (!munslime(mdef, TRUE) && !DEADMONSTER(mdef)) {
                /* this assumes newcham() won't fail; since hero has
                   a slime attack, green slimes haven't been bani'd */
        ...
        if (mattk->aatyp == AT_WEAP) {
            ...
            if (Role_if(PM_ARCHEOLOGIST)) {
                ...
                if (touching_artifacts)
                    mhm->damage += rn2(2 * touching_artifacts) + rn2(2 * touching_artifacts);
            }
            if (Ferocious)
                mhm->damage += 10;
              ...
              struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (magr->data == &mons[PM_MASTER_OF_THIEVES]) {
            if (!rn2(5))
                stealamulet(magr);
        }
        else
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    case AD_MUMY: mhitm_ad_mumy(magr, mattk, mdef, mhm); break;
    default:
        mhm->damage = 0;
    }
}
    ...
    int specialdmg) /* blessed and/or silver bonus against various things */
{
    ...
    if (is_demon(gy.youmonst.data) && !rn2(13) && !uwep
        && u.umonnum != PM_BALROG) { // #if 0 && u.umonnum != PM_AMOROUS_DEMON #endif
        demonpet();
        return MM_MISS;
    }
    ...
    boolean weapon_used)  /* True: via weapon hit */
{
    ...
    /* if you are being held by a gigantic monster, you can't be pushed */
    if (u_def && u.ustuck && u.ustuck != magr &&
        (u.ustuck->data == &mons[PM_TARASQUE] || u.ustuck->data == &mons[PM_ORANGE_DRAGON]))
        return FALSE;
    ...
    if (u_def) {
        ...
        if (u.ustuck == magr)
            set_ustuck((struct monst *) 0);
        if (u.ustuck && (magr->data == &mons[PM_TARASQUE] || magr->data == &mons[PM_ORANGE_DRAGON])) {
            pline("%s is forced to release you!", Monnam(u.ustuck));
            set_ustuck((struct monst *) 0);
        }
        if (u.usteed)
...
hmonas(struct monst *mon)
{
    ...
    for (i = 0; i < NATTK; i++) {
        ...
        if (gs.skipdrin && mattk->aatyp == AT_TENT && mattk->adtyp == AD_DRIN)
            continue;
        if (Pinned && (mattk->aatyp == AT_WEAP || mattk->aatyp == AT_CLAW || mattk->aatyp == AT_TUCH))
            continue;
        weapon = 0;
        switch (mattk->aatyp) {
        case AT_WEAP:
        ...
        case AT_HUGS: {
            ...
            if (unconcerned) {
                ...
                if (specialdmg || mindless(mon->data)
                    || mon->mhp <= 1 + max(u.udaminc, 1) + 4 * touching_artifacts + (Ferocious ? 10 : 0))
                    unconcerned = FALSE;
            }
        ...
        boolean wep_was_destroyed)
{
    ...
    case AD_ACID:
        if (mhitb && rn2(2)) {
            ...
            if (!Acid_resistance)
                ...
                monstseesu(M_SEEN_ACID);
                mdamageu(mon, resist_damage(AD_ACID, tmp));
            }
            if (!rn2(30))
    ...
    if (malive && !mon->mcan && rn2(3)) {
        ...
        case AD_ELEC:
            if (Shock_resistance) {
                shieldeff(u.ux, u.uy);
                You_feel("quite a tingle.");
                monstseesu(M_SEEN_ELEC);
                ugolemeffects(AD_ELEC, tmp);
                tmp = resist_damage(AD_ELEC, tmp);
            } else {

...
mon_wield_item(struct monst *mon)
{
    ...
    case NEED_HTH_WEAPON:
        if (!Is_rogue_level(&u.uz) && is_elf(mon->data) && (obj = select_rwep(mon)) && MON_WEP(mon) && ammo_and_launcher(obj, MON_WEP(mon)) && obj->quan >= 5)
            return 0;
        obj = select_hwep(mon);
        break;
...
dbon(void)
{
    ...
    if(!strength_type) {
        ...
        if (str < 6)
            ...
            returndbon = 7;
        if (uwep && is_art(uwep, ART_SCEPTRE_OF_MIGHT) && Role_if(PM_CAVE_DWELLER) && returndbon > 0)
            return 2 * returndbon;
        return returndbon;
    }
}
...
static int
adj_pract(int skill, int skilldecrement)
{
    int i = P_SKILL(skill) - skilldecrement;
    if (!i)
        return 0; // i = 1;
    int j = P_MAX_SKILL(skill);
    if (!j)
        j = 1;
    if (i < j)
        return practice_needed_to_advance(i);
    return practice_needed_to_advance(i + (i + 1 - j));
}
static int
adjusted_practice_needed_to_advance(int skill)
{
    if (P_MAX_SKILL(skill) <= P_EXPERT && P_SKILL(skill) >= P_EXPERT)
        return adj_pract(skill, 1);
    else if (P_SKILL(skill) == P_GRAND_MASTER)
        return adj_pract(skill, 1);
    int i = P_SKILL(skill);
    if (!i)
        i = 1;
...
can_advance(int skill, boolean speedy)
{
    ...
    if (wizard && speedy)
        return TRUE;
    if ((int) P_ADVANCE(skill) < practice_needed_to_advance(P_SKILL(skill)))
        return FALSE;
    int i = P_SKILL(skill);
    if (!i)
        i = 1;
    ...
    if (!j)
        j = 1;
    if (i >= j && (int) P_ADVANCE(skill) < practice_needed_to_advance(i + (i + 1 - j)))
        return FALSE;
    return (boolean) u.weapon_slots >= slots_required(skill);
}
...
{
    return FALSE; // the new system is a bit more flexible
...
skill_advance(int skill)
{
    ...
    if (P_MAX_SKILL(skill) <= P_EXPERT && P_SKILL(skill) >= P_EXPERT)
        You("are now most skilled in %s.", P_NAME(skill));
    else if (P_SKILL(skill) == P_GRAND_MASTER)
        You("are now most skilled in %s.", P_NAME(skill));
    else
                ...
                if (wizard) {
                    if (!iflags.menu_tab_sep)
                        Snprintf(buf, sizeof(buf), " %s%-*s %-12s %5d(%4d)", prefix,
                                 ...
                                 adjusted_practice_needed_to_advance(i));
                    else
...
drain_weapon_skill(int n) /* number of skills to drain */
{
    ...
    while (--n >= 0) {
        if (u.skills_advanced) {
            ...
            curradv = adj_pract(skill, 0);
            prevadv = adj_pract(skill, 1);
...
weapon_hit_bonus(struct obj *weapon)
{
    ...
    if (type == P_NONE) {
        ...
        if (weapon == uswapwep) {
            ...
            if (!skilllevel)
                skilllevel = 1;
            schar skillclass = objects[weapon->otyp].oc_skill;
            // this can't actually happen
            /* if (skillclass == -P_BOW || skillclass == -P_CROSSBOW)
                skillclass = P_DART; */
            switch (skillclass) {
                // case P_DART:
                // case P_SHURIKEN:
                ...
                case P_WHIP:
                case P_SHORT_SWORD:
                // case P_UNICORN_HORN: // (two-handed)
                    bonus -= (P_EXPERT - skilllevel + 2);
                    break;
                default:
                    bonus -= 2 * (P_EXPERT - skilllevel + 2);
                    break;
            }
        }
        if (P_SKILL(wep_type) < P_BASIC)
            bonus -= 4;

...
dig(void)
{
    ...
    if (Role_if(PM_ARCHEOLOGIST)) // we don't need to add number of touching artifacts to digging effort because archeologists can anyway dig instantly
        gc.context.digging.effort = 251;
    if (gc.context.digging.down) {
        struct trap *ttmp = t_at(dpx, dpy);

        if (gc.context.digging.effort > 250 || (ttmp && ttmp->ttyp == HOLE)) {
            (void) dighole(FALSE, FALSE, (coord *) 0, 0);
        ...
        if (dighole(TRUE, FALSE, (coord *) 0, 0)) {
...
void
digactualhole(coordxy x, coordxy y, struct monst *madeby, int ttyp, boolean notnow)
{
    ...
    if (ttyp == PIT) {
        if (madeby_u) {
            ...
            if (at_u)
                pline("%s creates a fissure in the %s below you!", Monnam(madeby), surface_type);
            else
        ...
        if (at_u) {
            if (!wont_fall) {
                ...
                if (!madeby_u && !madeby_obj && canseemon(madeby))
                    You("fall in.");
                if (!madeby_u && !((!madeby_obj && canseemon(madeby)))
                    && cansee(x, y) && Verbose(0, digactualhole1))
                    You("fall in.");
        ...
        if (madeby_u)
            ...
            if (at_u)
                pline("%s expands the fissure in the %s below you!", Monnam(madeby), surface_type);
            else
...
boolean
dighole(boolean pit_only, boolean by_magic, coord *cc, boolean only_checking)
{
    register struct trap *ttmp;
    struct rm *lev;
    ...
    if (only_checking)
        if (IS_FOUNTAIN(old_typ) || IS_SINK(lev->typ))
            return FALSE;
    ...
    if ((ttmp && (undestroyable_trap(ttmp->ttyp) || nohole))
        ...
        if (only_checking)
            return FALSE;
        digactualhole(dig_x, dig_y, BY_YOU, PIT, 0);
        dig_up_grave(cc);
        retval = TRUE;
    ...
    /* the following two are here for the wand of digging */
        ...
        if (typ != ROOM) {
            ...
            if (nohole || pit_only)
                ...
                digactualhole(dig_x, dig_y, BY_YOU, HOLE, 0);
            retval = TRUE;
        }
    }
    ...
    if (u.dz) {
        if (!Is_airlevel(&u.uz) && !Is_waterlevel(&u.uz) && !Underwater) {
            if (u.dz < 0 || On_stairs(u.ux, u.uy)) {
                ...
                (void) dighole(FALSE, TRUE, (coord *) 0, 0);
            }
        }
        return;
    } /* up or down */
    ...
    if ((Teleportation || can_teleport(gy.youmonst.data))
        ...
        if (amorphous(gy.youmonst.data) || Passes_walls
            ...
            good = (tunnels(gy.youmonst.data) && !needspick(gy.youmonst.data))
                      ? dighole(TRUE, FALSE, (coord *)0, 0) : TRUE;
            if (good)
                unearth_you();
        }
    }

...
savebones(int how, time_t when, struct obj *corpse)
{
 ...
 make_bones:
    ...
    if (u.ugrave_arise == (NON_PM - 1)) {
        ...
        gi.in_mklev = FALSE;
        if (!mtmp) { /* arise-type might have been banished */
            drop_upon_death((struct monst *) 0, (struct obj *) 0, u.ux, u.uy);
...
getbones(void)
{
    ...
    return 0; // bones are saved but loading them makes the game less fair
    ...
    if (validate(nhfp, gb.bones) != 0) {
        ...
        if (strcmp(bonesid, oldbonesid) != 0) {
             ...
             * subject to banishment, their mhpmax will be

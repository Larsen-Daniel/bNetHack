...
/*
 char firstline[1000];
char *bufferedlines[100];
int bufferedattr[100];

void
clearbuffer(void)
{
    int i;
    if (actual_arrow_count == 1)
        sprintf(firstline,"A glowing arrow emerges from the Longbow of Diana.");
    else
        sprintf(firstline,"%d glowing arrows emerge from the Longbow of Diana.", actual_arrow_count);
    putstr(WIN_MESSAGE, ATR_NONE, firstline);
    for (i = 0; i < buffer_len; i++) {
        if(i == 0 && actual_arrow_count == 1 && strncmp(bufferedlines[0],"The 1st",7) == 0) {
            sprintf(firstline,"The%s",bufferedlines[0]+7);
            putstr(WIN_MESSAGE, bufferedattr[i], firstline);
        }
        else
            putstr(WIN_MESSAGE, bufferedattr[i], bufferedlines[i]);
        free(bufferedlines[i]);
    }
    buffer_len = -1;
    buffer_print_please = 0;
}
*/
...
putmesg(const char *line)
{
    ...
    if ((gp.pline_flags & SUPPRESS_HISTORY) != 0
        ...
        attr |= ATR_NOHISTORY;
    /* if (buffer_len != -1) {
        if(buffer_print_please) // this should never happen but just in case
        {
            clearbuffer();
            ...
            if (buffer_len >= 100)
                ...
                bufferedlines[buffer_len] = strdup(line);
                bufferedattr[buffer_len] = attr;
                buffer_len++;
            }
        }
    }

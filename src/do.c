    ...
    boolean portal)    /* True if arriving via magic portal */
{
    ...
    if (Inhell && up && u.uhave.amulet && !newdungeon && !portal
        && (dunlev(&u.uz) < dunlevs_in_dungeon(&u.uz) - 3) && mysterious_force) {
    ...
    obj_delivery(FALSE);
    losedogs();
    kill_banished_monsters(); /* for those wiped out while in limbo */
    /*
...
zombify_mon(anything *arg, long timeout)
{
    ...
    if (zmon != NON_PM && !(gm.mvitals[zmon].mvflags & G_BANID)) {
        if (has_omid(body))
            free_omid(body);
        if (has_omonst(body))
            free_omonst(body);
...
void
set_pinned_arms(int timex)
{
    /* gc.context.botl = 1;
    if (!Pinned)
        ATEMP(A_DEX)--; */

    if (!Pinned || (Pinned & TIMEOUT) < (long) timex)
        set_itimeout(&Pinned, (long) timex);
}
...
heal_arms(void)
{
    if (Pinned && !nohands(gy.youmonst.data)) {
        ...
        const char *arms = body_part(ARM);
        arms = makeplural(arms);
        You("regain control of your %s.", arms);
    }

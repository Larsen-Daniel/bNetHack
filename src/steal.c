    ...
    if (monkey_business) {
        ...
        if (ostuck || can_carry(mtmp, otmp) == 0) {
            static const char *const how[] = { "steal", "snatch", "grab",
                                               "take" };
 cant_take:
            if ((otmp->oartifact == ART_ORB_OF_DETECTION && Role_if(PM_ARCHEOLOGIST)) || (otmp->oartifact == ART_ORB_OF_FATE && Role_if(PM_VALKYRIE)))
            {
                pline("%s reaches for %s%s.", Monnam(mtmp),
                  (otmp->owornmask & W_ARMOR) ? "your " : "",
                  (otmp->owornmask & W_ARMOR) ? equipname(otmp)
                                              : yname(otmp));
                pline("The floating orb easily dodges.");
            }
            else
...
stealamulet(struct monst* mtmp)
{
    ...
    if (otmp)
        if ((otmp->oartifact == ART_ORB_OF_DETECTION && Role_if(PM_ARCHEOLOGIST)) || (otmp->oartifact == ART_ORB_OF_FATE && Role_if(PM_VALKYRIE)))
                if (rnd(5)!=1)
                {
                    pline("%s resists being stolen by %s", The(xname(otmp)), mon_nam(mtmp));
                    return;
                }

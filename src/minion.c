...
msummon(struct monst *mon)
{
    ...
    if (is_dprince(ptr) || (ptr == &mons[PM_WIZARD_OF_YENDOR])) {
        ...
        dtype = (!rn2(20)) ? dlord(atyp) : ndemon(atyp, ptr);
        cnt = 1;
        ...
        if (!rn2(6)) {
            ...
            case A_CHAOTIC:
            case A_NONE:
                dtype = ndemon(atyp, (struct permonst *) 0);
                break;
            }
        } else {
            dtype = PM_ANGEL;
    ...
    /* sanity checks */
    if (cnt > 1 && (mons[dtype].bani & G_UNIQ) != 0)
        cnt = 1;
    /*
...
summon_minion(aligntyp alignment, boolean talk)
{
    ...
    case A_CHAOTIC:
    case A_NONE:
        mnum = ndemon(alignment, (struct permonst *) 0);
        break;
    default:
        impossible("unaligned player?");
        mnum = ndemon(A_NONE, (struct permonst *) 0);
        break;
    }
    if (mnum == NON_PM) {
        mon = 0;
...
dlord(aligntyp atyp)
{
    ...
    return ndemon(atyp, (struct permonst *) 0); /* approximate */
}
...
int
ndemon(aligntyp atyp, struct permonst *summoner) /* A_NONE is used for 'any alignment' */
{
    int tryct;
    struct permonst *ptr;
    ...
    if (summoner) {
        ...
        else if (summoner == &mons[PM_AFRIT]) {
            if (rn2(4))
                return NON_PM;
            if (i == 0)
                return  PM_DJINNI;
        ...
        else if (summoner == &mons[PM_DJINNI]) {
            if (rn2(8))
                return NON_PM;
            if (i == 4 && !rn2(5))
                return PM_MARID;
        ...
        else if (summoner == &mons[PM_RAKSHASA]) {
            if (i == 0)
                return  PM_PISHACHA;
            if (i != 4)
                return PM_RAKSHASA;
        }
        else if (summoner == &mons[PM_PISHACHA]) {
            if (i == 0)
                return  PM_RAKSHASA;
            if (i != 4)
                return PM_PISHACHA;
        }
        else if (summoner == &mons[PM_ONI]) {
            if (rn2(6))
                return PM_ONI;
        }
        else if (summoner == &mons[PM_UTUKKU]) {
            if (rn2(6))
                return PM_UTUKKU;
        }
        else if (summoner == &mons[PM_BALROG]) {
            if (i == 0 && rn2(2))
                return PM_BALROG;
            if (i > 0)
                return NON_PM;
        }
        else if ((summoner->bani & G_UNIQ) == 0) {
            if (rn2(6))
                return NON_PM;
        }
        else {

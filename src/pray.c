...
in_trouble(void)
{
    ...
    if ((Pinned & TIMEOUT) && !nohands(gy.youmonst.data))
        return TROUBLE_PINNED;
...
#define ok_wep(o) ((o) && ((o)->oclass == WEAPON_CLASS || is_weptool(o)))
    ...
    if (objects[class_gift].oc_class == SPBOOK_CLASS || objects[class_gift].oc_class == TOOL_CLASS) {
        char bbuf[BUFSZ];
    ...
    {
        // if (!Upolyd) // logically it is hard to imagine a long worm being able to fly
        You("find that you now possess wings.");
        boolean already_flying = !!Flying;
        crowned_valkyrie = 1;
        float_vs_flight(); /* block flying if levitating */
        if(!already_flying)
            if(Flying) { // the player could be engulfed
        ...
        /* not an artifact, but treat like one for this situation */
        livelog_printf(LL_DIVINEGIFT,
                       "was bestowed with wings");
        class_gift = ART_MJOLLNIR; // anything but STRANGE_OBJECT
        obj = 0;
    }
    switch (u.ualign.type) {
    case A_LAWFUL:
...
pleased(aligntyp g_align)
{
    ...
    if (pat_on_head)
        ...
        case 7:
        case 8:
            if (u.ualign.record >= PIOUS && !u.uevent.uhand_of_elbereth && !Upolyd) {
                gcrownu();
                break;
            }
            /*FALLTHRU*/
    ...
    if (altaralign != u.ualign.type && highaltar) {
        ...
        if (u.ugangr) {
            ...
            if (u.ulevel > 2 && u.uluck >= 0
                && !rn2(10 + (2 * u.ugifts * nartifacts)) && sacrifice_artifact_number < 2) {
                otmp = mk_artifact((struct obj *) 0, a_align(u.ux, u.uy));
                if (otmp) {
                    sacrifice_artifact_number++;
                    char buf[BUFSZ];

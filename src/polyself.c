...
set_uasmon(void)
{
    ...
    else if (is_vampire_could_shift(gy.youmonst.data))
...
#define PROPSET(PropIndx, ON)                          \
    ...
    PROPSET(REFLECTING, (mdat == &mons[PM_STEEL_GOLEM]));
#undef PROPSET
...
polyman(const char *fmt, const char *arg)
{
    ...
    /* check whether player foolishly banished self while poly'd */
    if (ubanished()) {
        /* intervening activity might have clobbered banishment info */
        ...
        if (kptr != (struct kinfo *) 0 && kptr->name[0]) {
            ...
            Strcpy(gk.killer.name, "self-banishment");
        }
        dealloc_killer(kptr);
        done(BANISHED);
    }
...
polyself(int psflags)
{
    ...
    boolean forcecontrol = ((psflags & POLY_CONTROLLED) != 0),
            ...
            isvamp = (is_vampire_could_shift(gy.youmonst.data)
 ...
 do_merge:
            ...
            if (!(gm.mvitals[mntmp].mvflags & G_BANID)) {
 ...
 do_vampyr:
            if (mntmp < LOW_PM || (mons[mntmp].bani & G_UNIQ)) {
                mntmp = (gy.youmonst.data == &mons[PM_VAMPIRE_LEADER]
                         && !rn2(10)) ? PM_WOLF
                                      : !rn2(4) ? PM_FOG_CLOUD
                                                : PM_VAMPIRE_BAT;
                if (gy.youmonst.cham >= LOW_PM
                    && !is_vampire_could_shift(gy.youmonst.data) && !rn2(2))
                    mntmp = gy.youmonst.cham;
            }
    ...
    /* The below polyok() fails either if everything is banished, or if
...
polymon(int mntmp)
{
    ...
    if (gm.mvitals[mntmp].mvflags & G_BANID) { /* allow G_EXTINCT */
    ...
    if (Stoned && poly_when_stoned(&mons[mntmp])) {
        /* poly_when_stoned already checked stone golem banishment */
    ...
    if (Verbose(2, polymon)) {
        ...
        if (is_vampire_could_shift(uptr) || is_vampshifter(&gy.youmonst))
...
uasmon_maxStr(void)
{
    ...
    if (strongmonst(ptr)) {
        ...
        if (!strength_type)
            ...
            newMaxStr = R ? R->attrmax[A_STR] : live_H ? 19 : 18;
    } else {
...
dobreathe(void)
{
    ...
    if ((u.umonnum == PM_RED_DRAGON || u.umonnum == PM_WHITE_DRAGON) && !Is_waterlevel(&u.uz)) {
        int returnvalue = throwspell(TRUE);
        if (returnvalue == -1)
            return ECMD_CANCEL;
        if (returnvalue == 0)
            return ECMD_TIME;
        ...
        int damage;
        struct obj *pseudo;
        pseudo = mksobj((u.umonnum == PM_WHITE_DRAGON)
                        ? SPE_CONE_OF_COLD : SPE_FIREBALL, FALSE, FALSE);
        while (n--) {
            if (!u.dx && !u.dy && !u.dz) {
                ...
                explode(u.dx, u.dy,
                        (u.umonnum == PM_WHITE_DRAGON)
                           ? SPE_CONE_OF_COLD - SPE_MAGIC_MISSILE + 10
                           : SPE_FIREBALL - SPE_MAGIC_MISSILE + 10,
                        spell_damage_bonus(u.ulevel), 0,
                        (u.umonnum == PM_WHITE_DRAGON)
                           ? EXPL_FROSTY
                           : EXPL_FIERY);
            }
...
dopoly(void)
{
    ...
    if (is_vampire_could_shift(gy.youmonst.data) || is_vampshifter(&gy.youmonst)) {
...
mbodypart(struct monst *mon, int part)
{
    ...
    if ((part == HAND || part == HANDED)
        && (humanoid(mptr) && attacktype(mptr, AT_CLAW)
            ...
            // #if 0 && mptr != &mons[PM_AMOROUS_DEMON])) #endif
...
ugolemeffects(int damtype, int dam)
{
     ...
     */
    if (u.umonnum != PM_FLESH_GOLEM && u.umonnum != PM_IRON_GOLEM && u.umonnum != PM_STEEL_GOLEM)
        return;
    switch (damtype) {
    case AD_ELEC:
...
int
resist_damage(int damtype, int dam)
{
    int factor = 1;
    switch (damtype) {
    case AD_ELEC:
        if (u.umonnum == PM_FLESH_GOLEM)
            dam = 0;
        if (EShock_resistance || is_innate_to_current_form(&HShock_resistance))
            dam = 0;
        factor = 2;
        break;
    case AD_FIRE:
        if (u.umonnum == PM_IRON_GOLEM || u.umonnum == PM_STEEL_GOLEM)
            dam = 0;
        if (EFire_resistance || is_innate_to_current_form(&HFire_resistance))
            dam = 0;
        factor = 3;
        break;
    case AD_COLD:
        if (ECold_resistance || is_innate_to_current_form(&HCold_resistance))
            dam = 0;
        factor = 3;
        break;
    case AD_DRST: /* proxy for all poison attacks */
        if (EPoison_resistance || is_innate_to_current_form(&HPoison_resistance))
            dam = 0;
        factor = 5;
        break;
    case AD_ACID:
        if (EAcid_resistance || is_innate_to_current_form(&HAcid_resistance))
            dam = 0;
        factor = 5;
        break;
    }
...
/* True iff hero's role or race has been banished */
boolean
ubanished(void)
{
    return ((gm.mvitals[gu.urole.mnum].mvflags & G_BANID)
            || (gm.mvitals[gu.urace.mnum].mvflags & G_BANID));
}

/* how hero feels "inside" after self-banishment of role or race */
const char *
udeadinside(void)
{
    /* self-banishment used to always say "you feel dead inside" but that

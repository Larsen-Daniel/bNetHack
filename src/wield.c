...
#define erodeable_wep(optr)                             \
    ((optr)->oclass == WEAPON_CLASS || is_weptool(optr) \
     || (optr)->otyp == HEAVY_IRON_BALL || (optr)->otyp == IRON_CHAIN || (optr)->otyp == STEEL_CHAIN)
...
setuwep(struct obj *obj)
{
     ...
     */
    boolean wielded_smasher = u_wield_art(ART_OGRESMASHER);
    setworn(obj, W_WEP);
    ...
    if (uwep == obj
        && (u_wield_art(ART_OGRESMASHER)
            || is_art(olduwep, ART_OGRESMASHER) || wielded_smasher))
        gc.context.botl = 1;
    if (uwep == obj
        && (u_wield_art(ART_SCEPTRE_OF_MIGHT)
            || is_art(olduwep, ART_SCEPTRE_OF_MIGHT)) && strength_type && sceptre_strength && Role_if(PM_CAVE_DWELLER)) {
        if(u_wield_art(ART_SCEPTRE_OF_MIGHT))
            if(obj->spe > 0)
            {
                obj->known = 1;
                ABON(A_STR) += obj->spe;
            }
        if(is_art(olduwep, ART_SCEPTRE_OF_MIGHT))
            if(olduwep->spe > 0)
                ABON(A_STR) -= olduwep->spe;
        gc.context.botl = 1;
    }
...
/*
static void
setuswapwepviolently(struct obj *obj)
{
    if (uswapwep->oartifact)
        set_artifact_intrinsic(uswapwep, FALSE, W_SWAPWEP);
...
dowield(void)
{
 ...
 wielding:
    ...
    oldwep = uwep;
    if (wep && !wep->oartifact && twow && uwep && uwep->oartifact && uswapwep && uswapwep->oartifact)
        much_less_powerful = 2;
    result = ready_weapon(wep);
    if (flags.pushweapon && oldwep && uwep != oldwep)
        setuswapwep(oldwep);
    else if (twow && uswapwep && uswapwep->oartifact)
        set_artifact_intrinsic_extra(uswapwep, 0, W_SWAPWEP);
    much_less_powerful = 0;
    
    /*
    if (oldwep && uwep != wep && u.twoweap)
        setuswapwep(oldwep);
    // if (uswapwep && uswapwep->oartifact && u.twoweap)
        // set_artifact_intrinsic(uswapwep, FALSE, W_SWAPWEP);
    result = ready_weapon(wep);
    // if (flags.pushweapon && oldwep && uwep != oldwep)
        // setuswapwep(oldwep);
...
doswapweapon(void)
{
    ...
    if (u.twoweap && uswapwep && uswapwep->oartifact)
        set_artifact_intrinsic_extra(uswapwep, 1, W_SWAPWEP);
...
doquiver_core(const char *verb) /* "ready" or "fire" */
{
 ...
 already_quivered:
        ...
        if (was_twoweap && uwep && uwep->oartifact && uswapwep && uswapwep->oartifact)
            much_less_powerful = 2;
        setuwep((struct obj *) 0);
        if (was_twoweap && uswapwep && uswapwep->oartifact)
            set_artifact_intrinsic_extra(uswapwep, 0, W_SWAPWEP);
        much_less_powerful = 0;
        untwoweapon();
        was_uwep = TRUE;
           ...
           const char *verb) /* "rub",&c */
{
    ...
    if (uswapwep == obj) {
        ...
        if (twow && uwep && uwep->oartifact && uswapwep && uswapwep->oartifact) // we can assume the tool is not an artifact
            much_less_powerful = 2;
        if (will_weld(obj)) {
...
can_twoweapon(void)
{
    ...
    if (Upolyd && !could_twoweap(gy.youmonst.data))
        ...
        if (!uwep->oartifact) {
            pline("%s being held second to such a puny weapon!",
                  Yobjnam2(uswapwep, "resist"));
            return FALSE;
        }
        else {
            struct artifact *uwepartif = &artilist[(int) uwep->oartifact], *uswapwepartif = &artilist[(int) uswapwep->oartifact];
            if (uswapwepartif->spfx & SPFX_INTEL || (uswapwepartif->alignment != A_NONE && uswapwepartif->alignment != uwepartif->alignment)) {
                pline("%s being held second to your %s!",
                      Yobjnam2(uswapwep, "resist"), xname(uwep));
                return FALSE;
            }
        }
        return TRUE;
...
dotwoweapon(void)
{
    ...
    if (u.twoweap) {
        ...
        if(u_wield_art(ART_OGRESMASHER) || wielded_smasher)
            gc.context.botl = 1;
        set_artifact_intrinsic_extra(uswapwep, FALSE, W_SWAPWEP);
        update_inventory();
        return ECMD_OK;
    }
    ...
    if (can_twoweapon()) {
        ...
        if(u_wield_art(ART_OGRESMASHER) || wielded_smasher)
            gc.context.botl = 1;
        set_artifact_intrinsic_extra(uswapwep, TRUE, W_SWAPWEP);
        update_inventory();

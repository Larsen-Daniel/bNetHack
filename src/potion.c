...
static void peffect_serendipity(struct obj *);
static void peffect_ferocity(struct obj *);
...
{
    boolean Stinging = (uwep && (EWarn_of_mon & W_WEP) != 0L) || (uswapwep && (EWarn_of_mon & W_SWAPWEP) != 0L);
...
peffect_invisibility(struct obj *otmp)
{
    ...
    if (is_spell)
        ...
        incr_itimeout(&HInvis, rn1(10, 100 + 60 * bcsign(otmp)));
...
static void
peffect_serendipity(struct obj *otmp)
{
    if (Unserendipitous)
        You_feel("like some things may start going your way.");
    else
        You_feel("like things are going your way.");
    incr_itimeout(&HSerendipitous, rn1(2, 20 + 12 * bcsign(otmp)));
}
static void
peffect_ferocity(struct obj *otmp)
{
    You_feel("unstoppable!");
    incr_itimeout(&HFerocious, rn1(2, 20 + 12 * bcsign(otmp)));
}
...
peffects(struct obj *otmp)
{
    ...
    case POT_POLYMORPH:
        ...
        break;
    case POT_SERENDIPITY:
        peffect_serendipity(otmp);
        break;
    case POT_FEROCITY:
        peffect_ferocity(otmp);
        break;
    default:
...
potionhit(struct monst *mon, struct obj *obj, int how)
{
 ...
 do_illness:
        ...
        case POT_CONFUSION:
        case POT_BOOZE:
            if (!smartresist(mon, POTION_CLASS, 0, NOTELL))
                mon->mconf = TRUE;
            break;
        case POT_INVISIBILITY: {

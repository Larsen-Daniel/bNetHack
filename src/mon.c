    ...
    const char *msg)
{
    ...
    if (!mptr || mptr < &mons[LOW_PM] || mptr >= &mons[NUMMONS]) {
        ...
        if (chk_bani && (gm.mvitals[mndx].mvflags & G_BANID) != 0)
            impossible("banished %s in play (%s)",
...
make_corpse(struct monst *mtmp, unsigned int corpseflags)
{
    ...
    case PM_STEEL_GOLEM:
        num = d(2, 6);
        while (num--)
            obj = mksobj_at(STEEL_CHAIN, x, y, TRUE, FALSE);
        free_mgivenname(mtmp); /* don't christen obj */
        break;
    case PM_GLASS_GOLEM:
...
minliquid_core(struct monst* mtmp)
{
    ...
    if (mtmp->data == &mons[PM_GREMLIN] && (inpool || infountain) && rn2(3)) {
        ...
        return 0;
    } else if ((mtmp->data == &mons[PM_IRON_GOLEM] || mtmp->data == &mons[PM_STEEL_GOLEM]) && inpool) {
        ...
        else if (mtmp->data == &mons[PM_STEEL_GOLEM]) {
            mtmp->mhp -= 2 * rn1(25, 25);
            if (DEADMONSTER(mtmp)) {
                mondied(mtmp);
                if (DEADMONSTER(mtmp))
                    return 1;
...
m_calcdistress(struct monst *mtmp)
{
    ...
    if (mtmp->mfleetim && !--mtmp->mfleetim)
        mtmp->mflee = 0;
    if (mtmp->mprotlen && !--mtmp->mprotlen) {
        if (mtmp->mprot > 0) {
            mtmp->mprot--;
            if (mtmp->mprot > 0)
                mtmp->mprotlen = 10;
            if (!Blind && cansee(mtmp->mx, mtmp->my) && canspotmon(mtmp))
                pline("The %s haze around %s %s.", hcolor(NH_GOLDEN), mon_nam(mtmp),
                      mtmp->mprot ? "becomes less dense" : "disappears");
        }
    }
    else if (!mtmp->mprotlen)
        mtmp->mprot = 0;
 ...
 * 2 => monster died (it must have grown into a banished form, but
...
/* Give an intrinsic to monster */
void
actually_givit(struct monst *mtmp, int prop, boolean vis, boolean temporary)
{
    const char *msg = NULL;
    ...
    case COLD_RES:
        intrinsic = MR_COLD;
        if (temporary)
            msg = "%s looks quite warm, for now.";
        else
    ...
    case SLEEP_RES:
        intrinsic = MR_SLEEP;
        if (temporary)
            msg = "%s looks wide awake, for now.";
        else
    ...
    case SHOCK_RES:
        intrinsic = MR_ELEC;
        if (temporary)
            msg = "%s briefly crackles with static electricity.";
        else
    ...
    if (!intrinsic || (mtmp->data->mresists & intrinsic) || active_res(mtmp, prop))
        msg = (const char *) 0;

    if (intrinsic) {
        if (!active_res(mtmp, prop) || mtmp->mintrinsicstimeout[prop - 1] != 0) {
            mtmp->mintrinsics |= intrinsic;
            if (temporary)
                mtmp->mintrinsicstimeout[prop - 1] = gm.moves + rn1(10, 10);
            else
                mtmp->mintrinsicstimeout[prop - 1] = 0;
        }
    }
...
mon_givit(struct monst *mtmp, struct permonst *ptr)
{
    ...
    actually_givit(mtmp, prop, vis, FALSE);
}
            ...
            if ((poolok || is_pool(nx, ny) == wantpool)
                ...
                if ((ttmp = t_at(nx, ny)) != 0) {
                    ...
                    if ((ttmp->ttyp != RUST_TRAP
                         || mdat == &mons[PM_IRON_GOLEM] || mdat == &mons[PM_STEEL_GOLEM])
...
mm_2way_aggression(struct monst *magr, struct monst *mdef)
{
    ...
    if (ma->msound == MS_GUARDIAN && !mdef->mpeaceful && !(md->msound == MS_GUARDIAN))
        return ALLOW_M|ALLOW_TM;
    ...
    if ((is_elf(ma) || is_dwarf(ma) || ma == &mons[PM_HOBBIT]) && md == &mons[PM_BALROG])
        return ALLOW_M|ALLOW_TM;
    ...
    struct permonst *mptr) /* reflects mtmp->data _prior_ to mtmp's death */
{
    ...
    /* hero is thrown from his steed when it dies or gets banished */
    if (mtmp == u.usteed)
...
lifesaved_monster(struct monst* mtmp)
{
    ...
    if (lifesave) {
        ...
        surviver = !(gm.mvitals[monsndx(mtmp->data)].mvflags & G_BANID);
        mtmp->mcanmove = 1;
        mtmp->mfrozen = 0;
        ...
        if (!surviver) {
            /* banished monster can't be life-saved */
            if (cansee(mtmp->mx, mtmp->my))
                pline("Unfortunately, %s is still banished...",
                      mon_nam(mtmp));
            mtmp->mhp = 0;
        }
    }
...
mondead(struct monst *mtmp)
{
    ...
    if (is_vampshifter(mtmp)) {
        ...
        if (mndx >= LOW_PM && mndx != monsndx(mtmp->data)
            && !(gm.mvitals[mndx].mvflags & G_BANID)) {
            coord new_xy;
            char buf[BUFSZ];
    ...
    if (is_dlord(mtmp->data) || is_dprince(mtmp->data))
        You_feel("as if a million voices suddenly cried out in relief.");
    ...
    boolean was_swallowed) /* digestion */
{
    ...
    if (((bigmonst(mdat) || mdat == &mons[PM_LIZARD]) && !mon->mcloned)
        ...
        return TRUE;
    tmp = 2 + ((mdat->bani & G_FREQ) < 2) + verysmall(mdat);
...
monstone(struct monst* mdef)
{
    ...
    if ((int) mdef->data->msize > MZ_TINY
        || !rn2(2 + ((int) (mdef->data->bani & G_FREQ) > 2))) {
    ...
    int xkill_flags) /* 1: suppress message, 2: suppress corpse, 4: pacifist */
{
    ...
    if (mtmp->data->mlet == S_MUMMY && !rn2(10))
    {
        int nobj = 0;
        int onum;
        struct obj *ootmp;
        for (ootmp = gi.invent; ootmp; ootmp = ootmp->nobj) {
            /* gold isn't subject to being cursed or blessed */
            if (ootmp->oclass == COIN_CLASS)
                continue;
            nobj++;
        }
        if (nobj) {
            ...
            if (!ootmp || ootmp->cursed)
                ;
            else {
                You("shudder involuntarily as the mummy's body crumbles.");
                pline("A malignant aura surrounds you.");
                if (ootmp->oartifact && spec_ability(ootmp, SPFX_INTEL)
                    && rn2(10) < 8)
                    pline("%s!", Tobjnam(ootmp, "resist"));
                else if (ootmp->blessed)
                        unbless(ootmp);
                else
                    curse(ootmp);
                update_inventory();
    ...
    if (accessible(x, y) || is_pool(x, y)) {
        ...
        /* no longer */
        if (rn2(6) < 0 && !(gm.mvitals[mndx].mvflags & G_NOCORPSE)
...
wake_nearto(coordxy x, coordxy y, int distance)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (distance == 0 || dist2(mtmp->mx, mtmp->my, x, y) < distance) {
            ...
            if (mtmp->mfrozen && mtmp->mfrozenr == REASON_SLEEPING) {
                mtmp->mcanmove = 1;
                mtmp->mfrozen = 0;
            }
...
pick_flyer(void)
{
    ...
    short flyer_temp[SPECIAL_PM];
    int i, n;
    
    for (n = 0, i = LOW_PM; i < SPECIAL_PM; i++) {
        if (mons[i].mlet == S_BAT && i != PM_VAMPIRE_BAT)
            flyer_temp[n++] = i;
        if (mons[i].mlet == S_DRAGON && mons[i].msize > MZ_HUGE)
            flyer_temp[n++] = i;
        if (mons[i].mlet == S_NAGA && mons[i].msize >= MZ_HUGE)
            flyer_temp[n++] = i;
    }

    res = flyer_temp[rn2(n)];
    return res;
}
...
pick_scary(void)
{
    int min_target_level = monmax_difficulty_lev() + rn2(3);
    int max_target_level = min_target_level + rn2(3);
    
    short scary_temp[SPECIAL_PM];
    int i, n;
    
    for(int j = 0; j < 10; j++) {
        n = 0;
        for (i = LOW_PM; i < SPECIAL_PM; i++)
            if (mons[i].difficulty >= min_target_level && mons[i].difficulty <= max_target_level)
                if (rn2(3) == 0)
                    scary_temp[n++] = i;
        if (n > 2)
            return scary_temp[rn2(n)];
        min_target_level--;
        max_target_level++;
    }
    return PM_TARASQUE;
...
pickvampshape(struct monst* mon)
{
    ...
    if (gm.mvitals[mndx].mvflags & G_BANID)
        return mon->cham;
...
{
    if (base >= LOW_PM && is_vampire_could_shift(&mons[base])) {
...
select_newcham_form(struct monst* mon)
{
    ...
    case PM_IJIRAQ:
        ...
        do {
            ...
            if (validspecmon(mon, mndx) && mons[mndx].difficulty >= mons[PM_IJIRAQ].difficulty
                && mons[mndx].difficulty <= mons[PM_IJIRAQ].difficulty + rn2(6))
                if (tryct < 40 || !Is_rogue_level(&u.uz) || isupper((uchar) mons[mndx].mlet))
                    break;
        } while (--tryct > 0);
        if (tryct > 0)
            break;
        mndx = pick_nasty(mons[PM_MUMAK].difficulty - 1);
        break;
...
accept_newcham_form(struct monst *mon, int mndx)
{
    ...
    if (mndx == NON_PM)
        return 0;
    mdat = &mons[mndx];
    if ((gm.mvitals[mndx].mvflags & G_BANID) != 0)
        return 0;
    ...
    unsigned ncflags)
{
    ...
    if (mdat == 0) {
        ...
        if (!tryct)
            return 0;
    } else if (gm.mvitals[monsndx(mdat)].mvflags & G_BANID)
        return 0; /* passed in mdat is banished */
...
dead_species(int m_idx, boolean egg)
{
    ...
    if (m_idx < LOW_PM)
        return TRUE;
    /*
     * For monsters with both baby and adult forms, banishing either
    ...
    return (boolean) ((gm.mvitals[m_idx].mvflags & G_BANID) != 0
                      || (gm.mvitals[alt_idx].mvflags & G_BANID) != 0);
}

/* kill off any eggs of banished monsters */
static void
...
/* kill all members of banished species */
void
kill_banished_monsters(void)
{
    ...
    /*
     * Called during banishment, and again upon level change.  The latter
     ...
     *
     * Chameleon handling:
     *  1) if chameleons have been banished, destroy them
     *     regardless of current form;
     *  2) otherwise, force every chameleon which is imitating
     *     any banished species to take on a new form.
     */
    for (mtmp = fmon; mtmp; mtmp = mtmp2) {
        ...
        kill_cham = (mtmp->cham >= LOW_PM
                     && (gm.mvitals[mtmp->cham].mvflags & G_BANID));
        if ((gm.mvitals[mndx].mvflags & G_BANID) || kill_cham) {
...
golemeffects(register struct monst* mon, int damtype, int dam)
{
    ...
    if (mon->data == &mons[PM_FLESH_GOLEM]) {
        if (damtype == AD_ELEC)
            ...
            slow = 1;
    } else if (mon->data == &mons[PM_IRON_GOLEM] || mon->data == &mons[PM_STEEL_GOLEM]) {
        if (damtype == AD_ELEC)
            slow = 1;

...
create_maze(int corrwid, int wallthick, boolean rmdeadends)
{
    ...
    int start = above;
    int startcol, endcol;
    for (startcol = 0; startcol <= COLNO; startcol++)
        if (levl[startcol][start].typ != STONE)
            break;
    for (endcol = COLNO; endcol >= 0; endcol--)
        if (levl[endcol][start].typ != STONE)
            break;
    for (y = ROWNO; y > 0; y--) {
        for (x = 2; x <= (rdx * 2); x++) {
            if (levl[x][y].typ != STONE)
    ...
    if (shift > 0) {
        for (y = end; y >= start; y--)
            for (x = 0; x <= COLNO; x++)
                levl[x][y + shift] = levl[x][y];
        for (y = 0; y < shift; y++)
            for (x = 0; x <= COLNO; x++)
                levl[x][start + y] = levl[x][end + shift + 1];
        start = start + shift;
        end = end + shift;
    }
    if (shift < 0) {
        shift = -shift;
        for (y = start - shift; y < end - shift; y++)
            for (x = 0; x <= COLNO; x++)
                levl[x][y] = levl[x][y + shift];
        for (y = 0; y < shift; y++)
            for (x = 0; x <= COLNO; x++)
                levl[x][end + y + 1] = levl[x][end + shift + 1];
        start = start - shift;
        end = end - shift;
    }
    for (y = 1; y <= start; y++)
        for (x = startcol; x <= endcol; x++)
            levl[x][y].typ = (y == 1) ? HWALL : (gl.level.flags.corrmaze) ? CORR : ROOM;
    for (y = end; y < ROWNO - 1; y++)
        for (x = startcol; x <= endcol; x++)
            levl[x][y].typ = (y == ROWNO - 2) ? HWALL : (gl.level.flags.corrmaze) ? CORR : ROOM;
    for (x = startcol; x <= endcol; x++) {
        if (x == startcol + 1) {
            x = endcol - 1;
            continue;
        ...
        for (y = start + 1; y < end; y++)
            if (levl[x][y].typ != levl[x][start + 1].typ)
                break;
        if (y == end) {
            for (y = 1; y <= start; y++)
                levl[x][y].typ = levl[x][start + 1].typ;
            for (y = end; y < ROWNO - 1; y++)
                levl[x][y].typ = levl[x][start + 1].typ;
        }
    }
    for (x = startcol + 1; x < endcol; x++)
        if (levl[x][0].typ != HWALL || levl[x][1].typ != HWALL)
            break;
    if (x == endcol)
        for (x = startcol; x <= endcol; x++)
            levl[x][0].typ = STONE;
    for (x = startcol + 1; x < endcol; x++)
        if (levl[x][ROWNO - 2].typ != HWALL || levl[x][ROWNO - 1].typ != HWALL)
            break;
    if (x == endcol)
        for (x = startcol; x <= endcol; x++)
...
makegoodies(void) {
    ...
    for (x = rn1(1, 3); x; x--) {
        mazexy(&mm);
        (void) mkobj_at(rn2(3) ? GEM_CLASS : RANDOM_CLASS, mm.x, mm.y, TRUE);
    }
    for (x = rn1(1, 1); x; x--) {
        mazexy(&mm);
        (void) mkgoodobj_at(RANDOM_CLASS, mm.x, mm.y, TRUE, FALSE);
    }
    for (x = rn1(10, 2); x; x--) {
        mazexy(&mm);
    ...
    for (x = rn1(2, 1); x; x--) {
        mazexy(&mm);
        struct permonst *pm = mkclass(S_DEMON, 0);
        if (pm)
            (void) makemon(pm, mm.x, mm.y, NO_MM_FLAGS);
    }
    for (x = rn2(3); x; x--) {
        mazexy(&mm);
    ...
    int udnum = (int) u.uz.dlevel - 1;
    if (In_hell(&u.uz) && udnum > rn2(100))
        maketreasure();
    if (In_hell(&u.uz) && udnum > rn2(200))
        maketreasure();
}
...
maketreasure(void)
{
    ...
    int cpt = 0;
    do {
        x = rnd(gx.x_maze_max - 10) + 5;
        y = rnd(gy.y_maze_max - 10) + 5;
        if (levl[x - 2][y].typ == STONE || levl[x + 2][y].typ == STONE || levl[x][y - 2].typ == STONE || levl[x][y + 2].typ == STONE)
            ;
        else if (levl[x][y].typ == allowedtyp || levl[x][y].typ == ICE)
            break;
    } while (++cpt < 100);
    if (x >= 6 && y >= 6 && x <= gx.x_maze_max - 5 && y <= gy.y_maze_max - 5 && (levl[x][y].typ == allowedtyp || levl[x][y].typ == ICE))
        ;
    else {
        // this could in principle happen
        // impossible("maketreasure() failed");
        return;
    }
    if (levl[x - 2][y].typ == STONE || levl[x + 2][y].typ == STONE || levl[x][y - 2].typ == STONE || levl[x][y + 2].typ == STONE)
        return;
    struct monst *mtmp = makemon(&mons[PM_STEEL_GOLEM], x, y, NO_MM_FLAGS);
    if (mtmp) { // there might be some reason this square can't be occupied, so we have to check
        allowedtyp = levl[x][y].typ;
        mtmp->guardingsquare = 1;
        mtmp->mfixedx = x;
        mtmp->mfixedy = y;
        levl[x - 1][y - 1].typ = allowedtyp;
        levl[x - 1][y].typ = allowedtyp;
        levl[x - 1][y + 1].typ = allowedtyp;
        levl[x][y - 1].typ = allowedtyp;
        levl[x][y].typ = allowedtyp;
        levl[x][y + 1].typ = allowedtyp;
        levl[x + 1][y - 1].typ = allowedtyp;
        levl[x + 1][y].typ = allowedtyp;
        levl[x + 1][y + 1].typ = allowedtyp;
        struct obj *otmp = mksobj(CHEST, FALSE, FALSE);
        struct obj *addedobj;
        int i;
        for (i = 0; i < rn1(3, 2); i++) {
            addedobj = mkgoodobj(RANDOM_CLASS, TRUE, FALSE);
            add_to_container(otmp, addedobj);
            otmp->owt = weight(otmp);
        }
        otmp->olocked = 1;

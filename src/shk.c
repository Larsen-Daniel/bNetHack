...
static long getprice(struct obj *, boolean, struct monst *);
...
/* getobj callback for object to be price-checked */
static int
price_ok(struct obj *obj UNUSED)
{
...
doprice(void)
{
    ...
    obj = getobj("check the price of", price_ok, GETOBJ_NOFLAGS);
    if (!obj)
        return ECMD_CANCEL;
    if (!obj->dknown)
        You("don't know how much %s costs.", the(xname(obj)));
    else if (obj->oartifact)
        pline("%s seems priceless.", The(xname(obj)));
    else if (objects[obj->otyp].oc_minprice == -1)
        You("don't know how much %s costs.", the(xname(obj)));
    else if (objects[obj->otyp].oc_minprice == objects[obj->otyp].oc_maxprice)
        pline("%s probably costs about %ld zorkmids.", The(xname(obj)),
              objects[obj->otyp].oc_minprice);
    else
        pline("%s probably costs between %ld and %ld zorkmids.", The(xname(obj)), objects[obj->otyp].oc_minprice, objects[obj->otyp].oc_maxprice);
    return ECMD_OK;
}
...
{
    long tmp = getprice(obj, FALSE, shkp),
    ...
    int round_by = 1000;
    int random = (obj->otyp + shkp->m_id) % (tmp / 10 + 2);
    for(int j = 0; j < 3; j++) {
        if (round_increment >= round_by) {
            int needed_adjustment = round_increment - 1 - (tmp % round_increment);
            if (2 * (needed_adjustment - 1) < random)
                tmp += needed_adjustment;
        }
        round_by /= 10;
    }
    round_by = 1000;
    for(int j = 0; j < 3; j++) {
        if (round_increment >= round_by) {
            int needed_adjustment = tmp % round_increment;
            if (5 * (needed_adjustment - 1) < random)
                tmp -= needed_adjustment;
        }
        round_by /= 10;
    }
    if (tmp > 10)
        if (tmp % 10 == 4 || tmp % 10 == 6)
            tmp = tmp - (tmp % 10) + 5;
    if (tmp > 20) {
        if (tmp % 10 == 1)
            tmp = tmp - (tmp % 10);
        if (tmp % 10 == 3 || tmp % 10 == 7)
            tmp = tmp - (tmp % 10) + 5;
    }
    if (tmp > 40) {
        if (tmp % 10 == 2)
    ...
    if (tmp < 200)
        if (tmp % 10 == 9 && tmp % 100 != 99 && (obj->otyp + shkp->m_id) % 3 != 0)
            tmp++;
    if (tmp > 500 && tmp % 100 != 99 && tmp % 50 != 0) {
        tmp = tmp - (tmp % 50) + 50;
    }
    if (tmp > 1000 && tmp % 1000 != 999 && tmp % 100 != 0) {
        tmp = tmp - (tmp % 100) + 100;
    }
...
{
    long tmp, unit_price = getprice(obj, TRUE, shkp), multiplier = 1L, divisor = 1L;
...
static long
getprice(register struct obj* obj, boolean shk_buying, register struct monst* shkp)
{
    ...
    if (shkp) {
        long idno = (obj->otyp + shkp->m_id) % 20;
        long extra_adjustment = (idno * idno) % 20;
        extra_adjustment += 40L;
        tmp *= extra_adjustment;
        tmp /= 50L;
    }
    return tmp;
}

...
static const struct propname {
    ...
    { CURSE_BLOCKING, "weaponless protection from curses" },
    { SERENDIPITOUS, "serendipity" },
    { FEROCIOUS, "ferocious fighting" },
    { PINNED, "arms frozen in place" },
...
slimed_to_death(struct kinfo* kptr)
{
     ...
     * Temporarily unbanish if necessary.
     */
    ...
    gm.mvitals[PM_GREEN_SLIME].mvflags = save_mvflags & ~G_BANID;
    ...
    /* life-saved; even so, hero still has turned into green slime;
       player may have banished green slimes after being infected */
    if ((gm.mvitals[PM_GREEN_SLIME].mvflags & G_BANID) != 0) {
        char slimebuf[BUFSZ];
        ...
        Strcpy(slimebuf, "green slime has been banished...");
        ...
        done(BANISHED); /* [should it be done_timeout(BANISHED, SLIMED)?] */
...
nh_timeout(void)
{
    ...
    for (upp = u.uprops; upp < u.uprops + SIZE(u.uprops); upp++) {
        boolean intrinsic_timeout = FALSE;
        boolean extrinsic_timeout = FALSE;
        if ((upp->intrinsic & TIMEOUT) && !(--upp->intrinsic & TIMEOUT))
            intrinsic_timeout = TRUE;
        if (upp - u.uprops == SERENDIPITOUS)
            if ((upp->extrinsic & TIMEOUT) && !(--upp->extrinsic & TIMEOUT))
                extrinsic_timeout = TRUE;
        if (intrinsic_timeout || extrinsic_timeout) {
            ...
            case SICK:
                ...
                if ((m_idx = name_to_mon(gk.killer.name,
                    ...
                    if (type_is_pname(&mons[m_idx])) {
                        gk.killer.format = KILLED_BY;
                    } else if (mons[m_idx].bani & G_UNIQ) {
            ...
            case SERENDIPITOUS:
                if (intrinsic_timeout && extrinsic_timeout)
                    break;
                if (intrinsic_timeout) {
                    if (Unserendipitous)
                        You_feel("that the world is now against you.");
                    else
                        You_feel("that the world is no longer on your side.");
                }
                if (extrinsic_timeout) {
                    if (Serendipitous)
                        You_feel("that the world is now on your side.");
                    else
                        You_feel("that the world is no longer against you.");
                }
                break;
            case FEROCIOUS:
                if (!Ferocious)
                    You("no longer feel unstoppable.");
                break;
            case GLIB:
...
hatch_egg(anything *arg, long timeout)
{
    ...
    if (get_obj_location(egg, &x, &y, 0)) {
        ...
        if (!(mons[mnum].bani & G_UNIQ)
            && !(gm.mvitals[mnum].mvflags & (G_BANID | G_EXTINCT))) {

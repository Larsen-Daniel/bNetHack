...
int mysterious_force = 0; // not saved

int strength_type = 1;
int sceptre_strength = 0;
int special_conflict = 0;
int ranger_bonus = 0;
// int buffer_len = -1;
// int buffer_print_please = 0;
int actual_arrow_count = 0;
int sacrifice_artifact_number = 0;
int crowned_valkyrie = 0;
int wizard_in_tower = 1;
int silent_archeologist_mode = 0;
int last_powerful_message = 0;
int positive_message = 0; // this makes the behavior at the beginning of the game correct
int its_a_steal = 0;
int steal_powerful = 0;
int never_saved_before = 0;
/* int recompute = 0;
int twowepold;
struct obj dummy_obj;
struct obj *uwepold = &dummy_obj;
struct obj *uswapwepold = &dummy_obj; */
int super_silent_mode = 0;
int much_less_powerful = 0;
int perm_con_damage = 0;
long explevelvals[MAXULEV + 1];
int setupexplevelvals = 0;
int auto_wizard = 1;
int dilutive_force = 0;
int boots_bonus = 0;
int cloak_bonus = 0;
int helmet_bonus = 0;
...
void
save_miscellaneous(NHFILE *nhfp)
{
    if (nhfp->structlevel) {
        bwrite(nhfp->fd, (genericptr_t) &strength_type, sizeof strength_type);
        bwrite(nhfp->fd, (genericptr_t) &sceptre_strength, sizeof sceptre_strength);
        bwrite(nhfp->fd, (genericptr_t) &special_conflict, sizeof special_conflict);
        bwrite(nhfp->fd, (genericptr_t) &ranger_bonus, sizeof ranger_bonus);
        bwrite(nhfp->fd, (genericptr_t) &actual_arrow_count, sizeof actual_arrow_count);
        bwrite(nhfp->fd, (genericptr_t) &sacrifice_artifact_number, sizeof sacrifice_artifact_number);
        bwrite(nhfp->fd, (genericptr_t) &crowned_valkyrie, sizeof crowned_valkyrie);
        bwrite(nhfp->fd, (genericptr_t) &wizard_in_tower, sizeof wizard_in_tower);
        bwrite(nhfp->fd, (genericptr_t) &silent_archeologist_mode, sizeof silent_archeologist_mode);
        bwrite(nhfp->fd, (genericptr_t) &last_powerful_message, sizeof last_powerful_message);
        bwrite(nhfp->fd, (genericptr_t) &positive_message, sizeof positive_message);
        bwrite(nhfp->fd, (genericptr_t) &its_a_steal, sizeof its_a_steal);
        bwrite(nhfp->fd, (genericptr_t) &steal_powerful, sizeof steal_powerful);
        bwrite(nhfp->fd, (genericptr_t) &never_saved_before, sizeof never_saved_before);
        bwrite(nhfp->fd, (genericptr_t) &super_silent_mode, sizeof super_silent_mode);
        bwrite(nhfp->fd, (genericptr_t) &much_less_powerful, sizeof much_less_powerful);
        bwrite(nhfp->fd, (genericptr_t) &perm_con_damage, sizeof perm_con_damage);
        bwrite(nhfp->fd, (genericptr_t) explevelvals, sizeof explevelvals);
        bwrite(nhfp->fd, (genericptr_t) &setupexplevelvals, sizeof setupexplevelvals);
        bwrite(nhfp->fd, (genericptr_t) &auto_wizard, sizeof auto_wizard);
        bwrite(nhfp->fd, (genericptr_t) &dilutive_force, sizeof dilutive_force);
        bwrite(nhfp->fd, (genericptr_t) &boots_bonus, sizeof boots_bonus);
        bwrite(nhfp->fd, (genericptr_t) &cloak_bonus, sizeof cloak_bonus);
        bwrite(nhfp->fd, (genericptr_t) &helmet_bonus, sizeof helmet_bonus);
        
        /* bwrite(nhfp->fd, &recompute, sizeof recompute);
         bwrite(nhfp->fd, &twowepold, sizeof twowepold);
         bwrite(nhfp->fd, (genericptr_t) uwepold, sizeof uwepold);
         bwrite(nhfp->fd, (genericptr_t) uswapwepold, sizeof uswapwepold); */
    }
}

void
restore_miscellaneous(NHFILE *nhfp)
{
    if (nhfp->structlevel) {
        mread(nhfp->fd, (genericptr_t) &strength_type, sizeof strength_type);
        mread(nhfp->fd, (genericptr_t) &sceptre_strength, sizeof sceptre_strength);
        mread(nhfp->fd, (genericptr_t) &special_conflict, sizeof special_conflict);
        mread(nhfp->fd, (genericptr_t) &ranger_bonus, sizeof ranger_bonus);
        mread(nhfp->fd, (genericptr_t) &actual_arrow_count, sizeof actual_arrow_count);
        mread(nhfp->fd, (genericptr_t) &sacrifice_artifact_number, sizeof sacrifice_artifact_number);
        mread(nhfp->fd, (genericptr_t) &crowned_valkyrie, sizeof crowned_valkyrie);
        mread(nhfp->fd, (genericptr_t) &wizard_in_tower, sizeof wizard_in_tower);
        mread(nhfp->fd, (genericptr_t) &silent_archeologist_mode, sizeof silent_archeologist_mode);
        mread(nhfp->fd, (genericptr_t) &last_powerful_message, sizeof last_powerful_message);
        mread(nhfp->fd, (genericptr_t) &positive_message, sizeof positive_message);
        mread(nhfp->fd, (genericptr_t) &its_a_steal, sizeof its_a_steal);
        mread(nhfp->fd, (genericptr_t) &steal_powerful, sizeof steal_powerful);
        mread(nhfp->fd, (genericptr_t) &never_saved_before, sizeof never_saved_before);
        mread(nhfp->fd, (genericptr_t) &super_silent_mode, sizeof super_silent_mode);
        mread(nhfp->fd, (genericptr_t) &much_less_powerful, sizeof much_less_powerful);
        mread(nhfp->fd, (genericptr_t) &perm_con_damage, sizeof perm_con_damage);
        mread(nhfp->fd, (genericptr_t) explevelvals, sizeof explevelvals);
        mread(nhfp->fd, (genericptr_t) &setupexplevelvals, sizeof setupexplevelvals);
        mread(nhfp->fd, (genericptr_t) &auto_wizard, sizeof auto_wizard);
        mread(nhfp->fd, (genericptr_t) &dilutive_force, sizeof dilutive_force);
        mread(nhfp->fd, (genericptr_t) &boots_bonus, sizeof boots_bonus);
        mread(nhfp->fd, (genericptr_t) &cloak_bonus, sizeof cloak_bonus);
        mread(nhfp->fd, (genericptr_t) &helmet_bonus, sizeof helmet_bonus);
        
        /* mread(nhfp->fd, (genericptr_t) &recompute, sizeof recompute);
        mread(nhfp->fd, (genericptr_t) &twowepold, sizeof twowepold);
        mread(nhfp->fd, (genericptr_t) uwepold, sizeof uwepold);
        mread(nhfp->fd, (genericptr_t) uswapwepold, sizeof uswapwepold); */
    }
}
    ...
    boolean *displaceu)
{
    ...
    if (gc.context.forcefight || !mtmp->mundetected || sensemon(mtmp)
        ...
        *displaceu = ((mtmp->data == &mons[PM_DISPLACER_BEAST] || mtmp->data == &mons[PM_BABY_SHIMMERING_DRAGON] || mtmp->data == &mons[PM_SHIMMERING_DRAGON]) && !rn2(2)
 ...
 pull_free:
            ...
            case 3:
                if (!u.ustuck->mcanmove) {
                    ...
                    u.ustuck->mfrozen = 1;
                    u.ustuck->mfrozenr = REASON_MISCELLANEOUS;
...
lookaround(void)
{
    ...
    for (x = u.ux - 1; x <= u.ux + 1; x++)
        for (y = u.uy - 1; y <= u.uy + 1; y++) {
            ...
            if (avoid_moving_on_trap(x, y, infront)) { // infront && gc.context.run > 1))) {
                // if (gc.context.run == 1)
...
inv_weight(void)
{
    ...
    while (otmp) {
        if (otmp->oclass == COIN_CLASS)
            ...
            if(otmp->oartifact != ART_ORB_OF_DETECTION || !Role_if(PM_ARCHEOLOGIST))
                if(otmp->oartifact != ART_ORB_OF_FATE || !Role_if(PM_VALKYRIE))
                        wt += otmp->owt;
        otmp = otmp->nobj;
    }
    gw.wc = weight_cap();

...
struct obj *
mkgoodobj_at(char let, coordxy x, coordxy y, boolean artif, boolean minvornot)
{
    struct obj *otmp;

    otmp = mkgoodobj(let, artif, minvornot);
    place_object(otmp, x, y);
    return otmp;
}
...
struct obj *
mkgoodobj(int oclass, boolean artif, boolean minvornot)
{
    int i;
    long prob;
    struct obj *otmp;
    ...
    if (oclass == FOOD_CLASS)
        return mkobj(FOOD_CLASS, artif);
    if (artif && oclass == WEAPON_CLASS && rn2(100) < 3) {
        switch (rnd(3)) {
        case 1:
    ...
    if (oclass == GEM_CLASS) {
        if (!rn2(20))
            return mksobj(LUCKSTONE, TRUE, artif);
        prob = 0;
    ...
    else if (oclass == AMULET_CLASS || oclass == SPBOOK_CLASS || oclass == WEAPON_CLASS || (oclass == ARMOR_CLASS && rn2(100) < 70)) {
        prob = rnd(go.oclass_prob_totals[oclass]);
        i = gb.bases[oclass];
        while ((prob -= objects[i].oc_prob) > 0)
            ++i;
    }
    else if (oclass == ARMOR_CLASS || oclass == TOOL_CLASS) {
        prob = 0;
        for (i = gb.bases[oclass]; objects[i].oc_class == oclass; i++)
        ...
        while ((prob -= objects[i].oc_prob) > 0) {
            for (;;) {
                ...
                if (objects[i].oc_class != oclass) {
                    impossible("object generation failed");
                    i = gb.bases[oclass];
                    break;
                }
                if (objects[i].oc_magic)
        ...
        prob = 0;
        for (i = gb.bases[oclass]; objects[i].oc_class == oclass; i++) {
            cost = objects[i].oc_cost;
            if (i == WAN_DEATH) // these should not be too rare
                cost = 250;
            cost = (cost < 50) ? 1 : (cost / 50);
            prob += cost * cost * cost * objects[i].oc_prob;
        }
        prob = rnd(prob);
        i = gb.bases[oclass];
        cost = objects[i].oc_cost;
        if (i == WAN_DEATH)
            cost = 250;
        cost = (cost < 50) ? 1 : (cost / 50);
        while ((prob -= cost * cost * cost * objects[i].oc_prob) > 0) {
            ++i;
            if (objects[i].oc_class != oclass) {
                impossible("object generation failed");
                i = gb.bases[oclass];
    ...
    otmp = mksobj(i, TRUE, artif);
    if (oclass == WEAPON_CLASS || oclass == ARMOR_CLASS || i == PICK_AXE) {
        i = otmp->spe;
        if (!rn2(3))
            otmp->spe += rn2(3);
        if (!rn2(3)) {

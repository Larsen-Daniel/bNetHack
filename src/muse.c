...
#define nomore(x)       if (gm.m.has_defense == x) continue;
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj) {
        ...
        if (obj->otyp == SCR_TELEPORTATION
            && mtmp->mcansee && haseyes(mtmp->data) && literate(mtmp->data)
...
rnd_defensive_item(struct monst* mtmp)
{
    ...
    if (is_animal(pm) || attacktype(pm, AT_EXPL) || mindless(mtmp->data)
        ...
        return 0;
 try_again:
    ; /* no variable declaration immediately after label */
    int switched = rn2(8 + (difficulty > 3) + (difficulty > 6) + (difficulty > 8));
    if (!literate(pm)) {
        if (switched < 3)
            switched = 3;
        if (switched > 5)
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj) {
        if (!reflection_skip) {
            ...
            if (obj->otyp == WAN_FIRE && obj->spe > 0
                && literate(mtmp->data)
                && (!m_seenres(mtmp, M_SEEN_FIRE) || !rn2(3))) {
                gm.m.offensive = obj;
            ...
            if (obj->otyp == WAN_COLD && obj->spe > 0
                && literate(mtmp->data)
                && (!m_seenres(mtmp, M_SEEN_COLD) || !rn2(3))) {
                gm.m.offensive = obj;
            ...
            if (obj->otyp == WAN_LIGHTNING && obj->spe > 0
                && literate(mtmp->data)
                && (!m_seenres(mtmp, M_SEEN_ELEC) || !rn2(3))) {
                gm.m.offensive = obj;
...
rnd_offensive_item(struct monst* mtmp)
{
    struct permonst *pm = mtmp->data;
    int difficulty = mons[(monsndx(pm))].difficulty;
    int capped_difficulty = difficulty;
    if (capped_difficulty > 15)
        capped_difficulty = 15;

    if (is_animal(pm) || attacktype(pm, AT_EXPL) || mindless(mtmp->data)
        || pm->mlet == S_GHOST || pm->mlet == S_KOP)
        return 0;
    if (literate(pm)) {
        if (difficulty >= 15 && !rn2(35))
            return WAN_DEATH;
        else if (difficulty > 7 && !rn2(35 + 5 * (15 - capped_difficulty)))
            return WAN_DEATH;
    }
    int switched = rn2(9 - (difficulty < 4) + 2 * (difficulty > 6) + 2 * (difficulty > 10));
    if (!literate(pm)) {
        if (switched < 2)
            switched = 2;
        if (switched > 6)
...
rnd_misc_item(struct monst* mtmp)
{
    ...
    if (difficulty < 6 && !rn2(30))
        return (rn2(6) || !literate(pm)) ? POT_POLYMORPH : WAN_POLYMORPH;
    ...
    case 0:
        if (mtmp->isgd)
            return 0;
        return (rn2(6) || !literate(pm)) ? POT_SPEED : WAN_SPEED_MONSTER;
    case 1:
        if (mtmp->mpeaceful && !See_invisible)
            return 0;
        return (rn2(6) || !literate(pm)) ? POT_INVISIBILITY : WAN_MAKE_INVISIBLE;
    case 2:
...
mon_reflects(struct monst* mon, const char* str)
{
    ...
    if (orefl && orefl->otyp == SHIELD_OF_REFLECTION) {
        ...
        return TRUE;
               ...
               || mon->data == &mons[PM_CHROMATIC_DRAGON] || mon->data == &mons[PM_STEEL_GOLEM]) {
        /* Silver dragons only reflect when mature; babies do not */
        if (str)
            pline(str, s_suffix(mon_nam(mon)), "scales");
        return TRUE;
    }
    if (u.usteed && mon == u.usteed && ureflects((char *) 0, (char *) 0))
    {
        if (str)
            ureflects(str, "your");
        return TRUE;
...
ureflects(const char* fmt, const char* str)
{
    ...
    if (EReflecting & W_ARMS) {
        ...
        return TRUE;
    } else if (EReflecting & W_WEP || EReflecting & W_SWAPWEP) {
        /* Due to wielded artifact weapon */
        if (fmt && str)
        ...
        return TRUE;
    } else if (gy.youmonst.data == &mons[PM_STEEL_GOLEM]) {
        if (fmt && str)
            pline(fmt, str, "steel exterior");
        return TRUE;
    }
    return FALSE;
}
...
cures_sliming(struct monst *mon, struct obj *obj)
{
    ...
    if (obj->otyp == SCR_FIRE)
        return (haseyes(mon->data) && mon->mcansee && literate(mon->data) && !nohands(mon->data));

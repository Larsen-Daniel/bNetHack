...
do_mgivenname(void)
{
     ...
     */
    if ((mtmp->data->bani & G_UNIQ) && !mtmp->ispriest) {
    ...
    unsigned oflgs)   /* flags, mostly for artifact creation */
{
    ...
    if (obj->oartifact) {
        /* can't dual-wield with artifact as secondary weapon */ // well, now you can, but this is not worth fixing
        if (obj == uswapwep)
        {
            if (!uwep->oartifact)
                untwoweapon();
    ...
    boolean called)
{
    ...
    if (name_at_start && (article == ARTICLE_YOUR || !has_adjectives)) {
        if (mdat == &mons[PM_WIZARD_OF_YENDOR])
            ...
            article = ARTICLE_NONE;
    } else if ((mdat->bani & G_UNIQ) != 0 && article == ARTICLE_A) {
        article = ARTICLE_THE;
    }
...
#define BOGUSMONSIZE 100 /* arbitrary */
    ...
    do {
        name = rn2_on_display_rng(SPECIAL_PM + BOGUSMONSIZE - LOW_PM) + LOW_PM;
             ...
             && (type_is_pname(&mons[name]) || (mons[name].bani & G_NOGEN)));

...
use_stethoscope(struct obj *obj)
{
    ...
    if (nohands(gy.youmonst.data)) {
        ...
        Your("%s are pinned.", makeplural(body_part(ARM)));
        return ECMD_OK;
    } else if (!freehand()) {
        You("have no free %s.", body_part(HAND));
        return ECMD_OK;
...
#define SEENMON (MONSEEN_NORMAL | MONSEEN_SEEINVIS | MONSEEN_INFRAVIS)
    ...
    if (mtmp->msleeping) {
        ...
        mtmp->mconf = 1;
                           ...
                           // #if 0 || mtmp->data == &mons[PM_AMOROUS_DEMON])) { #endif
        if (vis) {
...
dorub(void)
{
    ...
    if (Pinned) {
        You("aren't able to rub anything with pinned %s.", makeplural(body_part(ARM)));
        return ECMD_OK;
    }
...
use_trap(struct obj *otmp)
{
    ...
    if (what) {
        if (what[strlen(what) - 1] == ' ')
            You_cant("set a trap %s%s!", what, makeplural(body_part(ARM)));
        else
...
do_break_wand(struct obj *obj)
{
    ...
    if (nohands(gy.youmonst.data)) {
        ...
        You_cant("break %s with pinned %s.", yname(obj), makeplural(body_part(ARM)));
        return ECMD_OK;
    } else if (!freehand()) {
...
doapply(void)
{
    ...
    if (Pinned) {
        You("aren't able to use or apply tools in your current shape.");
        return ECMD_OK;
    }

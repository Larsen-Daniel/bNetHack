...
newuexpval(int lev)
{
    ...
    return (long) returnvalue * (long) 5 * (long) 5 * (long) 5 * (long) 5 * (long) 5;
    // the old system
    /* if (lev < 20)
...
newuexp(int lev)
{
    if (!setupexplevelvals) {
        int i;
        explevelvals[0] = 0;
        for (i = 1; i <= MAXULEV; i++)
            explevelvals[i] = explevelvals[i - 1] + newuexpval(i);
        setupexplevelvals = 1;
    }
    if (lev < 0 || lev > MAXULEV)
        impossible("Strange level: %d", lev);
    return explevelvals[lev];
}
...
int
experience(register struct monst *mtmp, register int nk UNUSED)
{
    ...
    if (extra_nasty(ptr))
        tmp += (mtmp->m_lev * mtmp->m_lev * mtmp->m_lev) / 10;
    else
        tmp += (mtmp->m_lev * mtmp->m_lev * mtmp->m_lev) / 20;
    // practical effect is more experience points for high-level monsters
...
/* e.g., hit by vampire attack */
void
losemaxhp(int attklev)
{
    int num, uhpmin, olduhpmax;
    ...
    olduhpmax = u.uhpmax;
    uhpmin = minuhpmax(10); /* same minimum as is used by life-saving */
    num = 1 + (boolean) ( rn2(attklev) >= 10) + (boolean) ( rn2(attklev) >= 20);
    u.uhpmax -= num;
    if (u.uhpmax < uhpmin)
        setuhpmax(uhpmin);
    /* uhpmax might try to go up if it has previously been reduced by

            ...
            if (wp->w_mask & mask) {
                ...
                if (oobj) {
                    ...
                    if (wp->w_mask & ~W_QUIVER) {
                        ...
                        if (oobj->oartifact || oobj->otyp == AMULET_OF_YENDOR) {
                            if (wp->w_mask & W_SWAPWEP && !twow)
                                silent_archeologist_mode = 1;
                            else
                                found_oobj = 1;
                            if (obj && obj->oartifact)
                                silent_archeologist_mode = 1;
                            set_artifact_intrinsic(oobj, 0, mask);
                            if (silent_archeologist_mode)
                                silent_archeologist_mode = 0;
                        }
                    }
                ...
                *(wp->w_obj) = obj;
                if (obj) {
                    ...
                    if (wp->w_mask & ~(W_SWAPWEP | W_QUIVER)) {
                        ...
                        if (obj->oartifact || obj->otyp == AMULET_OF_YENDOR) {
                            if (found_oobj)
                                silent_archeologist_mode = 1;

...
m_initweap(register struct monst *mtmp)
{
    ...
    case S_HUMAN:
        if (is_mercenary(ptr)) {
            ...
            if (mm == PM_HIGH_ELF) {
                (void) mongets(mtmp, ELVEN_BOW);
                m_initthrow(mtmp, ELVEN_ARROW, 100);
            }
            if (mm == PM_ELVEN_MONARCH) {
    ...
    case S_HUMANOID:
        if (mm == PM_HOBBIT) {
            ...
            if (!rn2((mm == PM_DWARF_RULER) ? 1
                     : ((mm == PM_DWARF_LEADER) ? 2 : 4))) {
    ...
    int i, tmp2, weaponmonster = 0, sawweapon = 0;
    for (i = 0; i < NATTK; i++) {
    ...
    if (weaponmonster) {
            ...
            if (obj->oclass == WEAPON_CLASS || obj->otyp == PICK_AXE) {
                ...
                if (obj->spe <= 1)
                    if ((int) mtmp->m_lev > rn2(40))
                        obj->spe++;
                if (obj->spe <= 2)
                    if ((int) mtmp->m_lev > rn2(80))
                        obj->spe++;
                if (obj->spe <= 3)
                    if ((int) mtmp->m_lev > rn2(160))
                        obj->spe++;
                if (!rn2(10))
                    obj->spe += rn2(7) - 3;
            }
        ...
        if (((mons[monsndx(mtmp->data)].bani & G_UNIQ) && (int) mtmp->m_lev * (int) mtmp->m_lev > rn2(5000)) || (!(mons[monsndx(mtmp->data)].bani & G_UNIQ) && (int) mtmp->m_lev * (int) mtmp->m_lev > rn2(50000))) {
            if (mons[monsndx(mtmp->data)].maligntyp > A_NEUTRAL)
                otmp = mk_artifact((struct obj *)0, A_LAWFUL);
            else if(mons[monsndx(mtmp->data)].maligntyp < A_NEUTRAL)
                otmp = mk_artifact((struct obj *)0, A_CHAOTIC);
            else
                otmp = mk_artifact((struct obj *)0, A_NEUTRAL);
            if (otmp)
                mpickobj(mtmp, otmp); // hope an ogre doesn't get Ogresmasher
        }
    }
...
#undef add_ac
    ...
    case S_NYMPH:
        ...
        for (i = 0; i <= rn2(2) + rn2(2) + rn2(3) + rn2(5) - 5; i++) {
    ...
    case S_GIANT:
        if (ptr == &mons[PM_MINOTAUR]) {
            ...
            int gem_number = (int) (mtmp->m_lev / 2);
            if (gem_number > 1)
                gem_number = rn2(gem_number);
            for (cnt = gem_number; cnt; cnt--) {
    ...
    if (ptr == &mons[PM_DWARF])
        for (i = 0; i <= rn2(2) + rn2(2) + rn2(3) + rn2(5) - 6; i++) {
            otmp = mkobj(TOOL_CLASS, FALSE);
            (void) mpickobj(mtmp, otmp);
        }
    if (ptr == &mons[PM_DWARF_LEADER])
        for (i = 0; i <= rn2(2) + rn2(2) + rn2(3) + rn2(5) - 5; i++) {
            otmp = mkobj(TOOL_CLASS, FALSE);
            (void) mpickobj(mtmp, otmp);
        }
    if (ptr == &mons[PM_DWARF_RULER])
        for (i = 0; i <= rn2(2) + rn2(2) + rn2(3) + rn2(5) - 4; i++) {
            otmp = mkobj(TOOL_CLASS, FALSE);
            (void) mpickobj(mtmp, otmp);
        }
    ...
    if (ptr == &mons[PM_SOLDIER] && rn2(13))
        return;
    if ((int) mtmp->m_lev > rn1(85, 15))
        for (i = 0; i < rn2(2) + rn2(2) + rn2(3) + rn2(5) - 6; i++) {
            otmp = mkgoodobj(AMULET_CLASS, TRUE, TRUE);
            (void) mpickobj(mtmp, otmp);
        }
    if (likes_gold(ptr))
        if ((int) mtmp->m_lev > rn2(50) && (int) mtmp->m_lev > rn2(50)) {
            if (rn2(2)) {
                for (i = 0; i < rn2(2) + rn2(2) + rn2(3) + rn2(5) - 4; i++) {
...
propagate(int mndx, boolean tally, boolean ghostly)
{
    ...
    gone = (gm.mvitals[mndx].mvflags & G_GONE) != 0; /* bani'd|extinct */
    result = ((int) gm.mvitals[mndx].born < lim && !gone) ? TRUE : FALSE;

    /* if it's unique, don't ever make it again */
    if ((mons[mndx].bani & G_UNIQ) != 0 && mndx != PM_HIGH_CLERIC)
...
newmonhp(struct monst *mon, int mndx)
{
    ...
    if (is_golem(ptr)) {
        ...
        mon->mhpmax = mon->mhp = 3 * (ptr->mlevel - 16);
        mon->m_lev = mon->mhp / 4; /* approximation */
    } else if (ptr->mlet == S_DRAGON && mndx >= PM_GRAY_DRAGON) {
        /* adult dragons; N*(8+rnd(4)) before endgame, N*12 once there */
        basehp = (int) mon->m_lev; /* not really applicable; isolates cast */
        mon->mhpmax = mon->mhp = In_endgame(&u.uz) ? (12 * basehp)
                                 : (8 * basehp + d(basehp, 4));
    } else if (!mon->m_lev) {
    ...
    mmflags_nht mmflags)
{
    ...
    *mtmp = cg.zeromonst; /* clear all entries in structure */
    ...
    case S_LIGHT:
    case S_ELEMENTAL:
        if (mndx == PM_STALKER || mndx == PM_BLACK_LIGHT || mndx == PM_RAKSHASA) {
            mtmp->perminvis = TRUE;
            mtmp->minvis = TRUE;
        }
        break;
    ...
    if (anymon && !(mmflags & MM_NOGRP)) {
        if ((ptr->bani & G_SGROUP) && rn2(2)) {
            m_initsgrp(mtmp, mtmp->mx, mtmp->my, mmflags);
        } else if (ptr->bani & G_LGROUP) {
            if (rn2(3))
...
uncommon(int mndx)
{
    if (mons[mndx].bani & (G_NOGEN | G_UNIQ))
        return TRUE;
    ...
    if (Inhell)
        ...
        return (boolean) ((mons[mndx].bani & G_HELL) != 0);
}
...
rndmonst_adj(int minadj, int maxadj)
{
    ...
    for (mndx = LOW_PM; mndx < SPECIAL_PM; ++mndx) {
         ...
         */
        weight = (int) (ptr->bani & G_FREQ) + align_shift(ptr);
...
static boolean
mk_gen_ok(int mndx, unsigned mvflagsmask, unsigned banimask)
{
    struct permonst *ptr = &mons[mndx];

    if (gm.mvitals[mndx].mvflags & mvflagsmask)
        return FALSE;
    if (ptr->bani & banimask)
        return FALSE;
...
struct permonst *
mkclass_aligned(char class, int spc, /* special mons[].bani handling */
                aligntyp atyp)
{
    ...
    mv_mask = G_GONE; /* G_BANID | G_EXTINCT */
    if ((spc & G_IGNORE) != 0) {
        mv_mask = 0; /* mv_mask &= ~G_GONE; */
        /* G_IGNORE is not a mons[].bani mask so get rid of it now */
        spc &= ~G_IGNORE;
    }
...
/* like mkclass(), but excludes difficulty considerations; used when
   ...
   banished types are avoided but extinct ones are acceptable; we don't
...
mkclass_poly(int class)
{
    ...
    for (last = first; last < SPECIAL_PM && mons[last].mlet == class; last++)
        if (mk_gen_ok(last, G_BANID, gmask))
            num += mons[last].bani & G_FREQ;
    if (!num)
        return NON_PM;

    for (num = rnd(num); num > 0; first++)
        if (mk_gen_ok(first, G_BANID, gmask))
            num -= mons[first].bani & G_FREQ;
...
grow_up(struct monst *mtmp, struct monst *victim)
{
    ...
    if ((int) ++mtmp->m_lev >= mons[newtype].mlevel && newtype != oldtype) {
        ...
        if (gm.mvitals[newtype].mvflags & G_BANID) { /* allow G_EXTINCT */
            if (canspotmon(mtmp))

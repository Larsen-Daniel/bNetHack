...
enum ms_sounds {
    ...
    MS_BOOM     = 43,   /* deafening cry (utukku) */
};
...
/* for mons[].bani (constant during game) */
...
#define G_BANI          0x0020 /* can be banished */
...
/* note: G_IGNORE controls handling of mvitals[].mvflags bits but is
   passed to mkclass() as if it dealt with mons[].bani bits */
#define G_IGNORE        0x8000 /* for mkclass(), ignore G_BANID|G_EXTINCT */

/* for gm.mvitals[].mvflags (variant during game), along with G_NOCORPSE */
#define G_KNOWN         0x04 /* have been encountered */
#define G_BANID         0x02 /* have been banished */
#define G_EXTINCT       0x01 /* population control; create no more */
#define G_GONE          (G_BANID | G_EXTINCT)

...
#define is_poisonable(otmp)                         \
    ((otmp->oclass == WEAPON_CLASS                   \
     && objects[otmp->otyp].oc_skill >= -P_SHURIKEN \
     && objects[otmp->otyp].oc_skill <= -P_BOW) || is_art(otmp, ART_GRIMTOOTH))
...
#define u_wield_art(art) (is_art(uwep, art) || (u.twoweap && is_art(uswapwep, art)))

...
#define largemonst(ptr) ((ptr)->msize == MZ_LARGE)

#define pm_resistance(ptr, typ) (((ptr)->mresists & (typ)) != 0)

#define mon_resistancebits(mon) \
    ((mon)->data->mresists | (mon)->mextrinsics)
#define active_res(mon, prop) (((mon)->mintrinsics & (1 << (prop - 1))) \
    && ((mon)->mintrinsicstimeout[prop - 1] == 0 || (mon)->mintrinsicstimeout[prop - 1] > gm.moves))
#define resists_fire(mon) \
    (((mon_resistancebits(mon) & MR_FIRE) != 0) || active_res(mon, FIRE_RES))
#define resists_cold(mon) \
    (((mon_resistancebits(mon) & MR_COLD) != 0) || active_res(mon, COLD_RES))
#define resists_sleep(mon) \
    (((mon_resistancebits(mon) & MR_SLEEP) != 0) || active_res(mon, SLEEP_RES))
#define resists_disint(mon) \
    (((mon_resistancebits(mon) & MR_DISINT) != 0) || active_res(mon, DISINT_RES))
#define resists_elec(mon) \
    (((mon_resistancebits(mon) & MR_ELEC) != 0) || active_res(mon, SHOCK_RES))
#define resists_poison(mon) \
    (((mon_resistancebits(mon) & MR_POISON) != 0) || active_res(mon, POISON_RES))
#define resists_acid(mon) \
    (((mon_resistancebits(mon) & MR_ACID) != 0) || active_res(mon, ACID_RES))
#define resists_ston(mon) \
    (((mon_resistancebits(mon) & MR_STONE) != 0) || active_res(mon, STONE_RES))

#define immune_poisongas(ptr) ((ptr) == &mons[PM_TARASQUE] \
                               || (ptr) == &mons[PM_UTUKKU])
...
#define literate(ptr) (likes_magic(ptr) || is_covetous(ptr) \
    || (humanoid(ptr) && !is_animal(ptr))  || (ptr)->mlet == S_IMP \
    || (ptr) == &mons[PM_GOLDEN_NAGA] || (ptr) == &mons[PM_MINOTAUR])
...
#define unique_corpstat(ptr) (((ptr)->bani & G_UNIQ) != 0)
...
#define is_vampire_could_shift(ptr) ((ptr)->mlet == S_VAMPIRE && (ptr) != &mons[PM_VAMPIRE_MAGE])
...
#define nonliving(ptr) \
    (is_undead(ptr) || weirdnonliving(ptr))
...
#define rustsconsiderably(ptr) ((ptr) == &mons[PM_STEEL_GOLEM])

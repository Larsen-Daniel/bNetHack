...
#define m_seenres(mon, mask) (((mon)->seen_resistance & (mask)) && \
    (((mask) != AD_ELEC && (mask) != AD_FIRE && (mask) != AD_COLD && \
    (mask) != AD_DRST && (mask) != AD_ACID) || rn2(2)))
#define m_setseenres(mon, mask) ((mon)->seen_resistance |= (mask))
#define monstseesu_ad(adtyp) monstseesu(cvt_adtyp_to_mseenres(adtyp))

#define REASON_MISCELLANEOUS 0 // if mfrozen somehow gets set automatically, it should be miscellaneous
#define REASON_SLEEPING 1
#define REASON_PARALYSIS 2
#define REASON_EMPLOYED 3
...
struct monst {
    ...
    coordxy mfixedx, mfixedy; /* if the monster is guarding something, where? */
    short guardingsquare;   /* if the monster is guarding something */
#define MTSZ 4
    ...
    unsigned short mwantedintrinsics; /* low 8 correspond to mresists */
    long mintrinsicstimeout[8];
    unsigned long seen_resistance; /* M_SEEN_x; saw you resist an effect */
    int mspec_used;             /* monster's special ability attack timeout */
    int mprot;                  /* amount of protection */
    short mprotlen;             /* timeout for protection */
    ...
    Bitfield(mfrozen, 7);
    Bitfield(mfrozenr, 2);  /* track reason of frozenness, not a type of sleeping which can be disturbed */
...
#define helpless(mon) ((mon)->msleeping || !(mon)->mcanmove || ((mon)->mfrozen && (mon)->mfrozenr == REASON_SLEEPING))
